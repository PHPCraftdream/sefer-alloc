...
// via the global allocator on the first `current_node()` dispatch (the
// `Vec::push` in `mock::record` grows the heap-allocated call log). That is a
// TEST-INFRASTRUCTURE allocation, not a production M5 violation: in a real
// (non-mock) build the platform `current_node_impl` does not touch the global
// allocator at all. The M5 invariant is still checked in every non-mock feature
// configuration (`production,alloc-stats`, `numa-aware`, etc.) — this skip only
// avoids the false positive that arises when the mock backend replaces the
// real platform code under the cfg flag.
