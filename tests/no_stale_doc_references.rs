...
/// Item 87's card (`docs/correctness-open-items/TRACKED_ci_gate_coverage.md`, cited via the
...
fn correctness_item_87_sentinel_counts_agree() {
    ...
    let index_path = manifest
        .join("docs")
        .join("correctness-open-items")
        .join("TRACKED_ci_gate_coverage.md");
    let index_text = fs::read_to_string(&index_path)
        .expect("read docs/correctness-open-items/TRACKED_ci_gate_coverage.md");
    ...
    let (headline_n, headline_pos) = extract_first_number_after(&index_text, headline_marker)
        .unwrap_or_else(|| {
            panic!(
                "docs/correctness-open-items/TRACKED_ci_gate_coverage.md: item 87's headline marker \
        ...
        if let Some(rest) = after.strip_prefix("**") {
            ...
            if digits_end > 0 && rest[digits_end..].starts_with("** as of this re-derivation") {
                ...
                assert!(
                    ...
                    "docs/correctness-open-items/TRACKED_ci_gate_coverage.md: found MORE THAN ONE live-shaped \
    ...
    let (next_trigger_n, next_trigger_pos) = live_next_trigger.unwrap_or_else(|| {
        panic!(
            "docs/correctness-open-items/TRACKED_ci_gate_coverage.md: item 87's live 'Next trigger' sentinel \
    ...
    assert_eq!(
        ...
        "docs/correctness-open-items/TRACKED_ci_gate_coverage.md item 87 has DRIFTED WITHIN ITSELF: the \
    ...
    assert_eq!(
        headline_n, script_n,
        "docs/correctness-open-items/TRACKED_ci_gate_coverage.md item 87's headline ({headline_n}) does not \
...
/// Item 59a's card (`docs/correctness-open-items/TRACKED_platform_contracts.md`, cited via the
/// stable `docs/CORRECTNESS_OPEN_ITEMS.md` item-number convention — see
...
fn correctness_item_59a_hugetlb_real_sentinel_count_agrees() {
    ...
    // docs/correctness-open-items/TRACKED_platform_contracts.md, not inline in
    // docs/CORRECTNESS_OPEN_ITEMS.md (now a thin index).
    let card_path = manifest
        .join("docs")
        .join("correctness-open-items")
        .join("TRACKED_platform_contracts.md");
    let card_text = fs::read_to_string(&card_path)
        .expect("read docs/correctness-open-items/TRACKED_platform_contracts.md");
    ...
    let (card_test_n, _) =
        extract_first_number_after(&card_text, card_test_marker).unwrap_or_else(|| {
            panic!(
                "docs/correctness-open-items/TRACKED_platform_contracts.md: item 59a's marker \
    ...
    assert_eq!(
        ...
        "docs/correctness-open-items/TRACKED_platform_contracts.md item 59a claims **{card_test_n}** unique \
    ...
    assert_eq!(
        ...
        "docs/correctness-open-items/TRACKED_platform_contracts.md item 59a claims **{card_marker_n}** unique \
    ...
    let card_total_n: u64 = card_text[..total_pos]
        ...
        .unwrap_or_else(|| {
            panic!(
                "docs/correctness-open-items/TRACKED_platform_contracts.md: item 59a states \
    ...
    assert_eq!(
        ...
        "docs/correctness-open-items/TRACKED_platform_contracts.md item 59a's own sentence does not add up: \
