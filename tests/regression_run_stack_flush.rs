...
fn contiguous_accepted_pushes_one_descriptor() {
    ...
    assert_eq!(
        head,
        u32::MAX,
        "linked-list head must be NULL (no singletons)"
    );
...
fn non_contiguous_gap_produces_split_descriptors() {
    ...
    assert_eq!(
        drained.len(),
        2,
        "two contiguous sub-runs → two descriptors"
    );
    assert!(
        drained.iter().all(|d| d.count == 6),
        "each sub-run has 6 blocks"
    );
...
fn guards_skip_already_free_block() {
    ...
    assert!(
        core.dbg_is_free_for(already_free),
    ...
    assert_eq!(
        descriptors.len(),
        2,
        "two sub-runs around the skipped block"
    );
...
fn run_member_blocks_not_on_linked_list_drain() {
    ...
    assert_eq!(
        ...
        "drained set == flushed set (Ф3 equivalence)"
    );
    // No duplicates.
