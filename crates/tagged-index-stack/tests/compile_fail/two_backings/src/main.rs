//! API-boundary regression fixture. This file MUST FAIL TO COMPILE: `StackHead` is
...
//! implementor owns head AND links in ONE place, so the caller cannot use two
//! backings against one head. This pins the API boundary, not a safety
//! invariant: the hazard class itself is
