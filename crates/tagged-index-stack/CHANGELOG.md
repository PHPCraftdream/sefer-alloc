...
- **Off-by-default test instrumentation** — `#[doc(hidden)]` read-only/counter probes compile under `--cfg tagged_index_stack_test` or `--cfg loom`; raw CAS/write probes remain loom-only. They carry no semver guarantee and are absent from default builds entirely.
...
- **Pending measurement** — the arm64 wall-clock A/B of the link-ordering and CAS-strength candidates has not been dispatched yet (the CI gate and its ratio oracle are in place and will check the run's own output when it is); until then, no ordering/CAS change is made on that basis.
