...
pub const SEFER_TABLE: [usize; SEFER_N] = build_table::<SEFER_N>(&SEFER_PARAMS);
pub const SEFER_MAX: usize = SEFER_TABLE[SEFER_N - 1];
pub const SEFER_L: usize = size2class_len(SEFER_MAX, SEFER_MIN_BLOCK);
// `static`, not `const`: a `const` this size re-materializes at every use
// site (see the crate's own SizeClasses doc for why).
pub static SEFER_SC: SizeClasses<SEFER_N, SEFER_L> = SizeClasses::build(SEFER_PARAMS);
...
/// in `tests/builder.rs`).
pub const JUMP_A: (usize, usize) = (1025, 256);
pub const JUMP_B: (usize, usize) = (2049, 1024);
