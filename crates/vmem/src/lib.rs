...
//   one syscall (the fast path for `align <= 64 KiB`) or two syscalls
//   (the traditional path for larger alignments). `WINDOWS_RESERVE_COMMIT_SINGLE_CALLS`
//   and `WINDOWS_RESERVE_COMMIT_TWO_CALL_PAIRS` count each path separately for
//   parity/comparison against the Unix hit-rate story.
//
...
/// + [`WINDOWS_RESERVE_COMMIT_TWO_CALL_PAIRS`] (sum of both paths).
/// Diagnostic only.
#[cfg(feature = "bench-internals")]
#[cfg_attr(docsrs, doc(cfg(feature = "bench-internals")))]
#[must_use]
