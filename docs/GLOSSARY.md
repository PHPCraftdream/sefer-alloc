...
| **Ф0–Ф6** | PERF-3 run-encoded-freelist arc phases (Cyrillic "Ф" = phase): Ф1 storage/layout (`RunStack`, feature `alloc-runfreelist` — now **removed**), Ф2/Ф3 encode/drain, Ф5 go/no-go cost-benefit gate (NO-GO). The experiment's negative result is preserved as institutional memory. | [design/RUN_ENCODED_FREELIST_PLAN.md](design/RUN_ENCODED_FREELIST_PLAN.md), [perf/PERF3_RUN_FREELIST_EXPERIMENT.md](perf/PERF3_RUN_FREELIST_EXPERIMENT.md) |
