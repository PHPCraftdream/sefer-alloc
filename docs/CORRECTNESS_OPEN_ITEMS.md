...
59a. **[T] Linux half of item 59 — CLOSED for dispatch AND for the kernel's syscall-level acceptance; the memory-CONTENT question stays open (task #1160/F5, closed further by task #1164, kernel-acceptance assertion strengthened by task #1166/F5).**

    The Linux `MAP_HUGETLB` success branch now DOES execute in CI: the `aligned-vmem-hugetlb-real` job (`.github/workflows/ci.yml`, added by tasks #1151/#1152) configures a real `nr_hugepages=64` pool and hard-asserts, via a path-activation oracle (`ci_hugetlb_real_pool_oracle_refuses_ordinary_page_fallback`), that `reserve_aligned_huge` actually received a real `MAP_HUGETLB` grant rather than silently falling back to ordinary pages. That job also runs the huge-decommit tests (`decommit_capability.rs`, `reservation_decommit_contract.rs`) under that real grant, proving the eligible-range decommit dispatch REACHES the real `madvise(2)`/`MADV_DONTNEED` backend call. **Task #1164 closed the next layer:** `ci_hugetlb_real_pool_kernel_actually_accepts_eligible_madvise` (`decommit_capability.rs`) additionally hard-asserts `unix_madvise_successes()` actually increased — i.e. the kernel returned `0`, not `-1`, for that call, on the same real grant. **Task #1166 (F5) strengthened that assertion from `successes_after > successes_before` to `assert_eq!(successes_after, attempts_after)`** — with the counters reset to zero immediately before the decommit call, this is exactly item 59a's own originally-stated bar (`unix_madvise_successes() == unix_madvise_attempts() > 0`), not a weaker stand-in for it; the strict-`>` form would have stayed green under a hypothetical future two-call-per-decommit dispatch where one call succeeds and one fails (attempts +2, successes +1), silently masking a partial failure the equality form catches. **What remains open, precisely stated, so this card cannot be read as more than it is:** a `0` return from `madvise(2)` means the kernel ACCEPTED the call; it is still NOT proof the kernel returned physical backing on next access, and NOT proof the pages actually zero-fill on re-fault — memory CONTENT after decommit-then-recommit is never read back on this path, on any platform. `libc_madvise` (`src/os/unix.rs`) still discards the return value on every NON-bench-internals build (task #719's original design, unchanged) — the read-back exists only under `bench-internals`, which is why this proof is scoped to the one CI job that enables it.
    - **Status:** PARTIALLY CLOSED — dispatch, grant, AND kernel syscall-level acceptance all proven (the acceptance proof now an exact equality, not a bare inequality); memory-content proof remains open (and is out of scope for this crate's own test suite per the flakiness reasoning task #1160 already accepted for a readback-based test).
    - **Current-number-or-verdict (re-derived task #1189, 2026-08-20; the task-#1166 figures this field previously carried are kept below as the historical record):** `aligned-vmem-hugetlb-real` job runs **8** unique `test <name> ... ok` sentinels under a real `MAP_HUGETLB` grant, plus **4** unique literal `[oracle] ARMED: ...` output markers — **12** `grep -F` sentinel checks in total (`awk '/^  aligned-vmem-hugetlb-real:/,/^  aligned-vmem-miri:/' .github/workflows/ci.yml | grep -oE 'grep -F "[^"]*"' | sort -u | wc -l` → 12, split confirmed by direct listing: 8 test-shaped, 4 marker-shaped). **Task #1166's own count was 6 + 2 = 8 and was accurate then**; the job grew by one test-sentinel plus one marker in EACH of task #1174 (commit `2828e04`, the write→decommit→read-zero postcondition) and task #1189 (this task, the release attempt/success oracle). Recorded plainly because this field went stale the moment #1174 landed and NEITHER that task's agent NOR its zero-trust review caught it — the count for this ONE job lives here as well as in item 87's whole-file totals, which is the same one-number-in-two-places coupling task #1161 documented; a round adding a sentinel to this job must update BOTH. The two `unix_madvise_successes() == unix_madvise_attempts()` and release attempt/failure equalities are the asserted bars on this path. `unix_madvise_successes() == unix_madvise_attempts()` (not merely `>`) is now the asserted bar on this path. **CI mechanics fix, same task:** the job's shared `cargo test` invocation previously passed `-- --nocapture` to the WHOLE multi-test run to make the `[oracle] ARMED: ...` markers observable — this was corrupting the very `test <name> ... ok` sentinel lines it was meant to coexist with, because libtest's default parallel runner writes each such line as three separate writes from the aggregating thread, and an unsynchronized worker-thread `println!` (either marker) can land between them, splitting another test's sentinel line. Verified by a 400-run local counterfactual against a 9/10-test harness mirroring this job's shape (11/400 runs corrupted at least one sentinel line under the old default-parallel `--nocapture` shape; `--test-threads=1 --nocapture` makes a printing test's OWN line corruption deterministic instead of rare, confirmed 5/5, so it is not a fix). Fixed by running the six `test <name> ... ok` sentinels WITHOUT `--nocapture` (libtest captures a passing test's stdout by default, so the two `println!`s are swallowed and the "... ok" lines are written intact — verified 400/400 clean) and observing each `[oracle] ARMED: ...` marker from its own isolated `--exact <name> -- --nocapture` invocation (verified 400/400 clean each). See `.github/workflows/ci.yml`'s `aligned-vmem-hugetlb-real` step comment for the full counterfactual summary.
    - **Next trigger:** none remaining for the syscall-acceptance question this card tracks. A future round wanting the memory-CONTENT question closed would need a readback-based test — task #1160 already declined that route for flakiness reasons (OS refusal under memory pressure/cgroup limits is contract-tolerated in general, just not inside this one hard-configured job) and this task did not revisit that call; re-open only if a concrete, non-flaky readback design is proposed.
    - **Evidence:** `.github/workflows/ci.yml` `aligned-vmem-hugetlb-real` job; `crates/aligned-vmem/tests/decommit_capability.rs` `huge_aligned_range_takes_the_real_backend_path_not_the_skip_path`, `ci_hugetlb_real_pool_oracle_refuses_ordinary_page_fallback`, and `ci_hugetlb_real_pool_kernel_actually_accepts_eligible_madvise` (task #1164, its kernel-acceptance assert strengthened to `assert_eq!` by task #1166); `crates/aligned-vmem/src/os/unix.rs` `libc_madvise` (return-value discard on non-bench builds, task #719) and its `bench-internals` `UNIX_MADVISE_ATTEMPTS`/`UNIX_MADVISE_SUCCESSES` counters, `decommit_pages_impl` (:423-447, confirms exactly one `libc_madvise` call per eager decommit today, which is why the equality form is sound and not tautological-different from the old `>` form on the CURRENT dispatch); `crates/aligned-vmem/tests/smoke.rs` `macos_decommit_madvise_syscall_actually_succeeds` (the precedent pattern this task's test mirrors, Darwin-only).
