...
90. **[T, second instance of item 89's class — a NO-GO review landed with zero index cards] Commit `967b821` committed `docs/reviews/2026-08-19-aligned-vmem-publication-readiness-audit-r2.md` (independent read-only re-audit of `aligned-vmem`, verdict **NO-GO for publishing 0.2.0** — 0 Critical, 0 High, 4 Medium, 3 Low, 3 perf candidates, 1 coverage gap) and, per its own body, "filed as #1171 (decision), with #1172/#1173/#1174 sequenced behind it" — but filed no card in this index, `docs/perf/OPEN_ITEMS.md`, or either archive. Filed here (task #1175) exactly one round after item 89 named the same "filed as a follow-up" pattern in a different script, confirming it is a recurring class, not a one-off.** (Filed 2026-08-19, task #1175.)
    ...
    - **Status:** OPEN — owner has deferred all PUB-R2 remediation work until the OX3 review wave is triaged; no scope has been chosen yet. This card records the review's findings plus a separate `@fh` consultation (see below) so neither is lost the way item 89's target was.
    ...
    - **Owning follow-up tasks named by the landing commit (not yet started; this card is their index-of-record):** **#1171** — owner decision on M1+M3+M4 scope (blocked on the `@fh` recommendation below). **#1172** — M2 fix (structurally follows from whichever M1 option #1171 picks — see below). **#1173** — L1+L2+L3 fix (L3 redirects to item 88). **#1174** — strengthen the real-HugeTLB coverage gap (write→decommit→recommit/read-zero + RSS/pool observation).
    - **`@fh` consultation on M1/M3/M4 (session-only until this card; captured in full so it cannot be lost the way item 89's target was) — RECOMMENDATIONS, NOT YET A DECISION:**
      - Two audit premises were independently **checked and found wrong/overstated**: (1) "`Drop` silently loses the `munmap` error" is FALSE — under `bench-internals` it IS counted, in `UNIX_MUNMAP_FAILURES` (`crates/aligned-vmem/src/os/unix.rs:1103-1109`, task P2-6), so recommendation-point-4 of §M1 is already partly closed. (2) the crate-level "every entry point has two forms" text is an OVERSTATEMENT, not a MEDIUM bug: `crates/aligned-vmem/src/lib.rs:55` scopes the claim to "every **reservation/commit** entry point" — `decommit_lazy`/`release`/`leak_zeroed_pages` are out of that scope by the text's own wording; the real fix is a one-word precision edit (invert "infallible forms forward to try_*" for the decommit family), not a MEDIUM-severity defect.
      - **What was confirmed as a real, hard bug:** the `reservation_len` accuracy bullet (`crates/aligned-vmem/src/reservation.rs:1071-1078`) claims an under-count smaller than one runtime page is harmless — FALSE for HugeTLB, where a `munmap` length not a multiple of the huge-page size returns `EINVAL` and leaks the ENTIRE mapping. The crate already documents this exact mechanism for its OWN construction path (`crates/aligned-vmem/src/os/unix.rs:53-59`) but does not carry the huge-multiple requirement into `from_raw_parts`'s `# Safety`. The assert block at `crates/aligned-vmem/src/reservation.rs:1217-1229` checks only `PAGE` (4 KiB) multiples, never the 2 MiB huge-page size, even when `granted_huge == true`.
      - **Recommended direction (a hybrid, not either option the report itself offered):** keep `granted_huge: bool`, but narrow what `true` means to "mapping is in the crate's own 2-MiB format": (a) `from_raw_parts` asserts on Linux/Android that `len`, `reservation_len`, and `base − reservation` are all 2-MiB multiples; (b) gate accepting `granted_huge: true` behind the `huge-pages` feature (this closes M2 as a CONSEQUENCE — no feature, no huge-flagged adoption exists, so there is no divergent behavior to have); (c) rewrite the `reservation_len` rounding bullet to state the HugeTLB exception explicitly. Rejected alternatives: banning huge in `from_raw_parts` outright (breaks the documented `into_full_parts` → `from_raw_parts` round-trip that is the only host-independent CI coverage of the huge branch); typed huge-granularity metadata (over-engineering — the crate only ever implements `LINUX_HUGE_PAGE_SIZE = 2 MiB`, nothing else). M3 splits into a real `# Safety` section (memory-safety-only) plus a separate "Correctness contract" section — rejected alternative: declaring the tested violations UB and rewriting the tests, which would assert something demonstrably false (`tests/granted_huge_reader_enumeration.rs` pins the non-UB behavior). M4 recommended as `Result<DecommitOutcome, VmemError>` with `Skipped`/`Advised`/`Refused(VmemError)` variants behind one private helper — the crate already discards exactly this information today (`libc_madvise` reads `ret` and drops it); task #1164's `ci_hugetlb_real_pool_kernel_actually_accepts_eligible_madvise` proved by execution that the return value is available, so "the crate cannot know the outcome" is false — it throws the answer away. `Advised` would mean "kernel accepted the advice," explicitly NOT "RSS was reclaimed" (that gap is the coverage-gap item above, #1174).
      - **Recommended sequencing:** (1) M1-hybrid + M3 split in one pass (same doc block) + the M2 doc bug (bad `decommit_lazy` advice for `is_huge()` — `crates/aligned-vmem/src/reservation.rs:637-639`, `decommit_lazy` unconditionally skips the backend for huge mappings per `:891-895`); (2) M4's `DecommitOutcome` enum as its own phase; (3) L1/L2 after, once `DecommitOutcome`'s shape is settled. **Version: publish as 0.2.0, not 0.3.0** — 0.2.0 has never reached crates.io (crates.io currently has 0.1.0 under a different API), so burning straight to 0.3.0 would orphan CHANGELOG references ("alias removed in 0.3.0/1.0.0") for no semantic reason. **Known blast radius:** the M1-hybrid assert breaks `tests/reservation_decommit_contract.rs:405`'s `method_try_decommit_reports_malformed_range_on_huge_flagged_reservation` (not currently feature-gated — will fail in default rows post-fix; needs `#[cfg(feature = "huge-pages")]`; its sibling in `decommit_capability.rs:494` is already gated). `numa-shim` is unaffected (it always passes `granted_huge: false`). The `DecommitOutcome` enum is an estimated 300-500 line change across the four backend files plus one new `src/decommit_outcome.rs` (per this repo's "one file, one export" rule).
    - **Next trigger:** owner picks up PUB-R2 scope after the OX3 review wave is triaged (per the brief that filed this task); when that happens, task #1171 records the owner's actual M1/M3/M4 decision (this card's `@fh` block is a recommendation input to that decision, not the decision itself), and this card's Status line moves to reflect whichever of #1171/#1172/#1173/#1174 has landed, with commit SHAs, until all four are closed.
    - **Evidence:** `docs/reviews/2026-08-19-aligned-vmem-publication-readiness-audit-r2.md` (full report, 203 lines, commit `967b821`); `crates/aligned-vmem/src/os/unix.rs:53-59,1103-1109` (huge-munmap-granularity mechanism + the existing `UNIX_MUNMAP_FAILURES` counter under `bench-internals`); `crates/aligned-vmem/src/lib.rs:55` (the correctly-scoped "reservation/commit" entry-point claim); `crates/aligned-vmem/src/reservation.rs:637-639,891-895,1071-1078,1217-1229` (the stale `decommit_lazy` advice, the unqualified rounding bullet, and the `PAGE`-only assert); `tests/reservation_decommit_contract.rs:405` and `tests/decommit_capability.rs:494` (the test that would need gating under the M1-hybrid, and its already-gated sibling); `tests/granted_huge_reader_enumeration.rs` (pins the non-UB behavior M3's split preserves); item 53 above (the distinct, already-closed prior `granted_huge` finding, confirmed not a duplicate); item 88 above (owns L3/MSRV — not duplicated here).
