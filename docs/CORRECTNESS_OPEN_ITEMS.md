...
87. **[T, new static guard — record what it does and does NOT cover] `scripts/verify-ci-sentinels.mjs` (task #1150) statically checks every `grep -F "test <NAME> ... ok"` postcondition in `.github/workflows/ci.yml` against the real `#[should_panic]`/`#[ignore]` attributes on the `fn` it names — closing the class task #1146 (commit `77efd3a`) found and fixed by hand, but the guard has real, documented blind spots that a round-start reader must know before treating a green run as exhaustive.** (Filed 2026-08-19, task #1150.)
    ...
    - **Current-number-or-verdict (re-derived 2026-08-19, task #1164 — see the RE-DERIVE note below before trusting these figures on a later date):** the guard itself reports **40 sentinel(s) checked** (`node scripts/verify-ci-sentinels.mjs` → "OK — 40 sentinel(s) checked against their real `fn` attributes, all match."). Independently cross-checked against ci.yml's raw text: `grep -c 'grep -F' .github/workflows/ci.yml` = **48**, of which **8** are comment lines mentioning `grep -F` rather than real postcondition lines (`grep -n 'grep -F' .github/workflows/ci.yml | grep -cE '^[0-9]+:\s*#'`), leaving **40** real sentinel lines (48 − 8 = 40, matching the guard's own count) — currently at ci.yml lines 224, 244, 259, 411, 431, 782, 2006, 2069 for the 8 comments (unchanged from task #1162's derivation — this task's own new comments do not themselves mention the literal string `grep -F`). Of the 40 real sentinel lines: **34 unique names+markers** — **26 unique plain test names** (dedup: `records_reserve_and_decommit` / `reentrancy_is_silently_dropped_with_outer_record_intact` are each checked from 4 separate CI steps — the only two repeats; every other name appears exactly once), **6** unique `#[should_panic]` names (all six task #1146 fixed, each appearing exactly once), and **2** unique literal markers (`[oracle] ARMED: real MAP_HUGETLB grant confirmed` from task #1162, `[oracle] ARMED: kernel accepted eligible madvise(2) on real MAP_HUGETLB grant` from task #1164) — 26 + 6 + 2 = 34, independently confirmed by a Node.js dedup pass over the parsed sentinel bodies, not just a shell `sort -u` pipeline. 0 currently `#[ignore]`d among the sentinel-tracked names (`grep -rnE '^\s*#\[ignore\]' tests/ crates/*/tests/`, zero matches, unchanged from filing). Counterfactual re-verified at filing (not re-run in this pass): adding `#[should_panic]` to `crates/aligned-vmem/tests/mock.rs`'s `records_reserve_and_decommit` (a test whose sentinel is checked from 4 separate CI jobs/steps) makes the script report all 4 mismatches and exit 1; removing the attribute again restores exit 0.
