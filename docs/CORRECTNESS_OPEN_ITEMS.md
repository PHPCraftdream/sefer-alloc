...
79. **[T, record correction — closed on filing] Commit `2cedbbb`'s body (task #1113, the `&mut self` breaking change) understated the cost and justified it with a non-sequitur; the DECISION stands, the stated JUSTIFICATION was wrong and the count was off by one.** (Filed 2026-08-19, task #1123; every premise below re-verified on branch `vmem-p4-l`, not taken on the review's word.)
    ...
    - **Current-number-or-verdict, three corrections to one commit body:**
      ...
      3. **The other counts: a mostly clean bill, one figure flagged.** "Exactly TWO method-form call sites in `src/`" — CONFIRMED (both in `crates/aligned-vmem/src/lazy_reservation.rs`; `reservation.rs` itself has zero). "the root crate, `numa-shim`, `examples/` and `benches/` have ZERO" — CONFIRMED (`git show 2cedbbb --name-only` touches nothing outside `crates/aligned-vmem`). "59 test call sites across 5 files" — **CONFIRMED, twice, independently.** The agent that wrote this card reported a census of 56 (6/1/31/2/16) and honestly flagged it as a possible discrepancy rather than asserting the commit body wrong; that caution was right in method and wrong in result. Two independent counts agree on **59** with the SAME per-file breakdown 6/2/33/2/16 — the @oh reviewer's, and the orchestrator's re-run at merge:
         ...
         decommit_capability 6 · lazy_commit 2 · reservation_decommit_contract 33 · round_trip_contract 2 · smoke 16  =  59

      The 56-count is the outlier and its two low files (`lazy_commit` 1 vs 2, `reservation_decommit_contract` 31 vs 33) point at a receiver-shaped pattern (`\w+\.method\(`) missing forms where the receiver is not a bare identifier. Recorded in full rather than silently replaced, because an honest-but-wrong doubt entering an index as a correction is the exact failure mode this campaign keeps closing — and the only reason it did not is that the merge step re-ran the count instead of trusting the report.
