...
59a. **[T] Linux half of item 59 — CLOSED for dispatch AND for the kernel's syscall-level acceptance; the memory-CONTENT question stays open (task #1160/F5, closed further by task #1164).**

    The Linux `MAP_HUGETLB` success branch now DOES execute in CI: the `aligned-vmem-hugetlb-real` job (`.github/workflows/ci.yml`, added by tasks #1151/#1152) configures a real `nr_hugepages=64` pool and hard-asserts, via a path-activation oracle (`ci_hugetlb_real_pool_oracle_refuses_ordinary_page_fallback`), that `reserve_aligned_huge` actually received a real `MAP_HUGETLB` grant rather than silently falling back to ordinary pages. That job also runs the huge-decommit tests (`decommit_capability.rs`, `reservation_decommit_contract.rs`) under that real grant, proving the eligible-range decommit dispatch REACHES the real `madvise(2)`/`MADV_DONTNEED` backend call. **Task #1164 closed the next layer:** `ci_hugetlb_real_pool_kernel_actually_accepts_eligible_madvise` (`decommit_capability.rs`) additionally hard-asserts `unix_madvise_successes()` actually increased — i.e. the kernel returned `0`, not `-1`, for that call, on the same real grant. **What remains open, precisely stated, so this card cannot be read as more than it is:** a `0` return from `madvise(2)` means the kernel ACCEPTED the call; it is still NOT proof the kernel returned physical backing on next access, and NOT proof the pages actually zero-fill on re-fault — memory CONTENT after decommit-then-recommit is never read back on this path, on any platform. `libc_madvise` (`src/os/unix.rs`) still discards the return value on every NON-bench-internals build (task #719's original design, unchanged) — the read-back exists only under `bench-internals`, which is why this proof is scoped to the one CI job that enables it.
    - **Status:** PARTIALLY CLOSED — dispatch, grant, AND kernel syscall-level acceptance all proven; memory-content proof remains open (and is out of scope for this crate's own test suite per the flakiness reasoning task #1160 already accepted for a readback-based test).
    - **Current-number-or-verdict (task #1164, 2026-08-19):** `aligned-vmem-hugetlb-real` job is green and now runs 6 sentinel tests under a real `MAP_HUGETLB` grant (the prior 5, plus `ci_hugetlb_real_pool_kernel_actually_accepts_eligible_madvise`), plus 2 literal `[oracle] ARMED: ...` output markers (the prior 1, plus a new `[oracle] ARMED: kernel accepted eligible madvise(2) on real MAP_HUGETLB grant`). `unix_madvise_successes()`/`unix_madvise_attempts()` are now asserted on this path, closing this card's own prior next-trigger.
    ...
    - **Evidence:** `.github/workflows/ci.yml` `aligned-vmem-hugetlb-real` job (lines ~351-460); `crates/aligned-vmem/tests/decommit_capability.rs` `huge_aligned_range_takes_the_real_backend_path_not_the_skip_path`, `ci_hugetlb_real_pool_oracle_refuses_ordinary_page_fallback`, and `ci_hugetlb_real_pool_kernel_actually_accepts_eligible_madvise` (task #1164); `crates/aligned-vmem/src/os/unix.rs` `libc_madvise` (return-value discard on non-bench builds, task #719) and its `bench-internals` `UNIX_MADVISE_ATTEMPTS`/`UNIX_MADVISE_SUCCESSES` counters; `crates/aligned-vmem/tests/smoke.rs` `macos_decommit_madvise_syscall_actually_succeeds` (the precedent pattern this task's test mirrors, Darwin-only).
