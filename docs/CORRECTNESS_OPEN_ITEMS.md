...
87. **[T, new static guard — record what it does and does NOT cover] `scripts/verify-ci-sentinels.mjs` (task #1150) statically checks every `grep -F "test <NAME> ... ok"` postcondition in `.github/workflows/ci.yml` against the real `#[should_panic]`/`#[ignore]` attributes on the `fn` it names — closing the class task #1146 (commit `77efd3a`) found and fixed by hand, but the guard has real, documented blind spots that a round-start reader must know before treating a green run as exhaustive.** (Filed 2026-08-19, task #1150.)
    ...
    - **Current-number-or-verdict (re-derived 2026-08-19, task #1174, on top of commit `bf7b6b2` + this task's uncommitted changes — see the RE-DERIVE note below before trusting these figures on a later date; every command below was actually run this task, not carried forward):** the guard itself reports **42 sentinel(s) checked** (`node scripts/verify-ci-sentinels.mjs` → `[verify-ci-sentinels] OK — 42 sentinel(s) checked against their real \`fn\` attributes, all match.`); `MIN_SENTINEL_COUNT` in `scripts/verify-ci-sentinels.mjs` is **42** (`grep -n MIN_SENTINEL_COUNT.*= scripts/verify-ci-sentinels.mjs` → `const MIN_SENTINEL_COUNT = 42;`), so the guard's own count matches its floor exactly and emits neither the below-floor FAIL nor the task-#1161 above-floor WARNING. Independently cross-checked against ci.yml's raw text: `grep -c 'grep -F' .github/workflows/ci.yml` = **51** (moved from task #1169's 49 — task #1174's `aligned-vmem-hugetlb-real` addition added two new sentinel lines and zero new comment lines mentioning `grep -F`, see below), of which **9** are comment lines mentioning `grep -F` rather than real postcondition lines (`grep -n 'grep -F' .github/workflows/ci.yml | grep -cE '^[0-9]+:\s*#'`, unchanged from task #1169), leaving **42** real sentinel lines (51 − 9 = 42, matching the guard's own count; `grep -n 'grep -F' .github/workflows/ci.yml | grep -vE '^[0-9]+:\s*#' | wc -l` → 42 independently) — currently at ci.yml lines **224, 244, 259, 420, 439, 462, 871, 2095, 2158** for the 9 comments (shifted only by the line-number drift already present when this task started; re-confirmed by direct re-grep this task). Of the 42 real sentinel lines: **36 unique names+markers** — **27 unique plain test names** (dedup: `records_reserve_and_decommit` / `reentrancy_is_silently_dropped_with_outer_record_intact` are each checked from 4 separate CI steps — the only two repeats; every other name appears exactly once), **6** unique `#[should_panic]` names (all six task #1146 fixed, each appearing exactly once), and **3** unique literal markers (`[oracle] ARMED: real MAP_HUGETLB grant confirmed` from task #1162, `[oracle] ARMED: kernel accepted eligible madvise(2) on real MAP_HUGETLB grant` from task #1164, `[oracle] ARMED: decommitted range read back as all-zero on real MAP_HUGETLB grant` from task #1174) — 27 + 6 + 3 = 36, independently confirmed by a fresh Node.js dedup script this task, not just a shell `sort -u` pipeline. 0 currently `#[ignore]`d among the sentinel-tracked names (`grep -rnE '^\s*#\[ignore\]' tests/ crates/*/tests/`, zero matches, re-run this task, unchanged from filing). Counterfactual NOT re-run this task (last re-verified at task #1169 filing): adding `#[should_panic]` to `crates/aligned-vmem/tests/mock.rs`'s `records_reserve_and_decommit` (a test whose sentinel is checked from 4 separate CI jobs/steps) makes the script report all 4 mismatches and exit 1; removing the attribute again restores exit 0.
