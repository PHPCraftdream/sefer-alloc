...
92. **[T, coverage gap created BY a red-`main` fix — filed in the same commit that creates it, per the item-89/90 class lesson] `DecommitOutcome::Refused` has NO test coverage on Unix, on macOS, or under miri: its only test is now Windows-only, because the mechanism it used to provoke an OS refusal is non-deterministic OUTSIDE Windows by construction.** (Filed 2026-08-20, task #1199.)
    ...
    - **Status:** OPEN. **Current verdict:** `Refused` is exercised on Windows only, by `refused_variant_is_produced_by_a_genuine_os_refusal` (`crates/aligned-vmem/tests/decommit_outcome.rs`, gated `#[cfg(all(windows, not(miri), not(aligned_vmem_mock)))]`). Every other row — Linux, macOS, the BSDs, miri, the mock backend — has zero executable assertion that a backend refusal surfaces as `Refused` rather than `Advised`. **Next trigger:** anyone touching `dispatch_try_decommit`'s error path, or a decision to build the deterministic mechanism described below.
