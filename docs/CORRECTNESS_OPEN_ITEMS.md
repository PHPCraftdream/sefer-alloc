...
92. **[T, coverage gap created BY a red-`main` fix — filed in the same commit that creates it, per the item-89/90 class lesson] `DecommitOutcome::Refused` has NO test coverage on Unix, on macOS, or under miri: its only test is now Windows-only, because the mechanism it used to provoke an OS refusal is non-deterministic OUTSIDE Windows by construction.** (Filed 2026-08-20, task #1199.)
    ...
    - **The candidate fix, deliberately NOT applied blind:** reserve a region, release it, then `try_decommit` that just-released address while holding this file's `SERIAL` lock — deterministic within one test binary in a way a fixed `+64 MiB` offset never was. Not applied here because this project's development host is Windows: the Unix half could only be validated by pushing and watching CI, which is exactly how the present red arrived. Whoever takes this should build it against a Linux runner, not reason it through.
