...
87. **[T, new static guard — record what it does and does NOT cover] `scripts/verify-ci-sentinels.mjs` (task #1150) statically checks every `grep -F "test <NAME> ... ok"` postcondition in `.github/workflows/ci.yml` against the real `#[should_panic]`/`#[ignore]` attributes on the `fn` it names — closing the class task #1146 (commit `77efd3a`) found and fixed by hand, but the guard has real, documented blind spots that a round-start reader must know before treating a green run as exhaustive.** (Filed 2026-08-19, task #1150.)
    ...
    - **Current-number-or-verdict (re-derived 2026-08-20, task #1189, on top of commit `2828e046706593d805bc08d2ba6e1e5a7a0bb82d` + this task's uncommitted changes — see the RE-DERIVE note below before trusting these figures on a later date; every command below was actually run this task, not carried forward):** the guard itself reports **45 sentinel(s) checked** (`node scripts/verify-ci-sentinels.mjs` → `[verify-ci-sentinels] OK — 45 sentinel(s) checked against their real \`fn\` attributes, all match.`); `MIN_SENTINEL_COUNT` in `scripts/verify-ci-sentinels.mjs` is **45** (`grep -n MIN_SENTINEL_COUNT.*= scripts/verify-ci-sentinels.mjs` → `const MIN_SENTINEL_COUNT = 45;`), so the guard's own count matches its floor exactly and emits neither the below-floor FAIL nor the task-#1161 above-floor WARNING. Independently cross-checked against ci.yml's raw text: `grep -c 'grep -F' .github/workflows/ci.yml` = **55** (moved from task #1189's 53 — task #1173/L1 added ONE new sentinel line to the `aligned-vmem-gates` job PLUS ONE new comment line that itself mentions `grep -F`, so the raw total moved by two while the REAL count moved by one; both halves re-measured, not inferred), of which **10** are comment lines mentioning `grep -F` rather than real postcondition lines (`grep -n 'grep -F' .github/workflows/ci.yml | grep -cE '^[0-9]+:\s*#'`, unchanged from task #1174), leaving **44** real sentinel lines (53 − 9 = 44, matching the guard's own count; `grep -n 'grep -F' .github/workflows/ci.yml | grep -vE '^[0-9]+:\s*#' | wc -l` → 44 independently) — currently at ci.yml lines **224, 244, 259, 420, 439, 462, 901, 2125, 2188** for the 9 comments (shifted only by the line-number drift already present when this task started; re-confirmed by direct re-grep this task). Of the 45 real sentinel lines: **39 unique names+markers** — **29 unique plain test names** (dedup: `records_reserve_and_decommit` / `reentrancy_is_silently_dropped_with_outer_record_intact` are each checked from 4 separate CI steps — the only two repeats; every other name appears exactly once), **6** unique `#[should_panic]` names (all six task #1146 fixed, each appearing exactly once), and **4** unique literal markers (`[oracle] ARMED: real MAP_HUGETLB grant confirmed` from task #1162, `[oracle] ARMED: kernel accepted eligible madvise(2) on real MAP_HUGETLB grant` from task #1164, `[oracle] ARMED: decommitted range read back as all-zero on real MAP_HUGETLB grant` from task #1174, `[oracle] ARMED: real MAP_HUGETLB release attempted exactly once and did not fail` from task #1189) — 28 + 6 + 4 = 38, independently confirmed by a fresh Node.js dedup script this task, not just a shell `sort -u` pipeline. 0 currently `#[ignore]`d among the sentinel-tracked names (`grep -rnE '^\s*#\[ignore\]' tests/ crates/*/tests/`, zero matches, re-run this task, unchanged from filing). Counterfactual NOT re-run this task (last re-verified at task #1169 filing, unrelated test): adding `#[should_panic]` to `crates/aligned-vmem/tests/mock.rs`'s `records_reserve_and_decommit` (a test whose sentinel is checked from 4 separate CI jobs/steps) makes the script report all 4 mismatches and exit 1; removing the attribute again restores exit 0. This task's OWN new sentinel counterfactual (mechanically distinct from that one): commenting out `WINDOWS_VIRTUALFREE_RELEASE_ATTEMPTS.fetch_add` in `crates/aligned-vmem/src/os/windows.rs` (the Windows sibling of this task's new `UNIX_MUNMAP_ATTEMPTS` fix) reproducibly turns `tests/smoke.rs`'s `windows_virtualfree_release_is_attempted_exactly_once_and_does_not_fail` red (`left: 0, right: 1`) — run and reverted this task; see that item's own perf-index cross-reference (item 57) for the full trace.
