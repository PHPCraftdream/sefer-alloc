...
87. **[T, new static guard — record what it does and does NOT cover] `scripts/verify-ci-sentinels.mjs` (task #1150) statically checks every `grep -F "test <NAME> ... ok"` postcondition in `.github/workflows/ci.yml` against the real `#[should_panic]`/`#[ignore]` attributes on the `fn` it names — closing the class task #1146 (commit `77efd3a`) found and fixed by hand, but the guard has real, documented blind spots that a round-start reader must know before treating a green run as exhaustive.** (Filed 2026-08-19, task #1150.)
    ...
    - **Current-number-or-verdict (re-derived 2026-08-19, task #1169, on commit `bf7b6b2` — see the RE-DERIVE note below before trusting these figures on a later date; every command below was actually run this task, not carried forward):** the guard itself reports **40 sentinel(s) checked** (`node scripts/verify-ci-sentinels.mjs` → `[verify-ci-sentinels] OK — 40 sentinel(s) checked against their real \`fn\` attributes, all match.`); `MIN_SENTINEL_COUNT` in `scripts/verify-ci-sentinels.mjs` is **40** (`grep -n MIN_SENTINEL_COUNT.*= scripts/verify-ci-sentinels.mjs` → `const MIN_SENTINEL_COUNT = 40;`), so the guard's own count matches its floor exactly and emits neither the below-floor FAIL nor the task-#1161 above-floor WARNING. Independently cross-checked against ci.yml's raw text: `grep -c 'grep -F' .github/workflows/ci.yml` = **49** (moved from task #1164's 48 — task #1166/#1167's `aligned-vmem-hugetlb-real` rewrite added one new comment line mentioning `grep -F`, see below; the 40 real sentinel lines themselves did not change), of which **9** are comment lines mentioning `grep -F` rather than real postcondition lines (`grep -n 'grep -F' .github/workflows/ci.yml | grep -cE '^[0-9]+:\s*#'`), leaving **40** real sentinel lines (49 − 9 = 40, matching the guard's own count; `grep -n 'grep -F' .github/workflows/ci.yml | grep -vE '^[0-9]+:\s*#' | wc -l` → 40 independently) — currently at ci.yml lines **224, 244, 259, 420, 439, 462, 835, 2059, 2122** for the 9 comments (shifted from task #1164's 224/244/259/411/431/782/2006/2069 — task #1166 (`6ba1f31`) rewrote the `aligned-vmem-hugetlb-real` job's step and added two new explanatory comments that themselves mention `grep -F`, at what are now lines 439 and 462, which is why the comment count rose 8 → 9 and every later comment shifted). Of the 40 real sentinel lines: **34 unique names+markers** — **26 unique plain test names** (dedup: `records_reserve_and_decommit` / `reentrancy_is_silently_dropped_with_outer_record_intact` are each checked from 4 separate CI steps — the only two repeats; every other name appears exactly once — re-verified by `grep -n 'grep -F "test records_reserve_and_decommit' .github/workflows/ci.yml` and the same for the reentrancy name, both returning exactly 4 lines each this task), **6** unique `#[should_panic]` names (all six task #1146 fixed, each appearing exactly once), and **2** unique literal markers (`[oracle] ARMED: real MAP_HUGETLB grant confirmed` from task #1162, `[oracle] ARMED: kernel accepted eligible madvise(2) on real MAP_HUGETLB grant` from task #1164) — 26 + 6 + 2 = 34, independently confirmed by a standalone Node.js script (`node -e '...'` reading `.github/workflows/ci.yml`, matching `^\s*grep -F "([^"]*)"`, deduping into a `Set`) run fresh this task, not just a shell `sort -u` pipeline. 0 currently `#[ignore]`d among the sentinel-tracked names (`grep -rnE '^\s*#\[ignore\]' tests/ crates/*/tests/`, zero matches, re-run this task, unchanged from filing). Counterfactual NOT re-run this task (last re-verified at filing): adding `#[should_panic]` to `crates/aligned-vmem/tests/mock.rs`'s `records_reserve_and_decommit` (a test whose sentinel is checked from 4 separate CI jobs/steps) makes the script report all 4 mismatches and exit 1; removing the attribute again restores exit 0.
      - **[task #1169] F2 fix — this card is the R30-8-class defect it warns readers about, and had already happened once by the time this bullet was written.** Two prior commits each claimed, in their OWN commit message, "all three agree" on one number for `MIN_SENTINEL_COUNT` / the guard's reported count / "the card" — but "the card" in both cases actually held TWO DIFFERENT numbers for the same quantity at the same time, because this bullet's headline and the "Next trigger" bullet below are updated on different schedules. `971ca05`'s commit message says "the card states 38"; the card `971ca05` itself committed reads 38 in this bullet's headline but still read **37** in the Next-trigger bullet below (`git show 971ca05:docs/CORRECTNESS_OPEN_ITEMS.md | grep -o "re-derivation established ([0-9]* as of"` → `37`). `cecdeec`'s commit message says "card states 40"; the card `cecdeec` committed reads 40 in this headline but left the Next-trigger bullet's "37" UNTOUCHED (same grep against `cecdeec` → `37`, unchanged — confirmed independently this task, matching the reviewer's own reproduction). Both commit-message claims of "the card states N" were therefore false in the narrow sense that the card contained N in one place and a stale different number in another; this task's fix (updating the Next-trigger bullet's figure to match this headline, immediately below) closes that specific gap, not the general possibility of the two going out of sync again — see the "structural" note in the Next-trigger bullet for why full structural elimination was not done.
    ...
    - **Next trigger:** if a new CI job introduces a `grep -F "test ... ok"` sentinel shape this guard's parser does not recognize (e.g. a step structure other than `- run: <cmd>` / `- run: |` / `- name: X` + `run: |`, all three of which are handled today), the script's own malformed-sentinel path already fails loudly rather than silently skipping — but a genuinely new STEP SHAPE the parser's `splitIntoRunSteps` regex does not match at all would silently produce zero sentinels found for that step, which is why `verify-ci-sentinels.mjs`'s own checked-count is printed on every run (`node scripts/verify-ci-sentinels.mjs` prints "N sentinel(s) checked") — a round noticing the printed N drop UNEXPECTEDLY (i.e. with no corresponding sentinel removal in ci.yml) below whatever N the round's own re-derivation established (**40** as of this re-derivation, task #1169, `node scripts/verify-ci-sentinels.mjs` — a fixed threshold like "below 30" would itself go stale as the count grows, so compare against the freshly re-run count, not a number copied from this card) is the signal to re-examine the parser. **Partly superseded by task #1157/F12:** a count drop is now ALSO a hard `MIN_SENTINEL_COUNT` assertion failure, so it no longer depends on a human noticing the printed number. The floor is a pinned integer and must be RAISED in the same commit that legitimately adds a sentinel row — a floor left behind is a floor that stops firing.
      - **[task #1169] F2 — this bullet's own figure (37) had drifted from this card's headline figure (40) before this fix, which is exactly the failure mode this bullet warns a round-start reader about.** `971ca05` raised this bullet's number 33 → 37 while its OWN commit's headline read 38 (not reconciled); `cecdeec` then raised the headline 38 → 40 while leaving this bullet's number at 37 (also not reconciled) — both commits' own messages claimed "the card states N" as if the card held one number, when it held two. Fixed here by copying the headline's freshly re-derived 40 into this bullet, in the same task that also fixed the headline. **Structural question considered: should this bullet stop restating the number at all, and instead say "compare against the freshly re-run count" without a copied figure?** Rejected for THIS bullet specifically — a reader deciding "is 40 → 37 a real drop or has the round's own count moved past 40" needs SOME anchor for what the round considered current at documentation time, and the bullet already says, in the same sentence, "compare against the freshly re-run count, not a number copied from this card" — the number here is a snapshot for orientation, not the enforced source of truth (that role belongs to `MIN_SENTINEL_COUNT`, a hard-asserted floor in the script itself, which this bullet already defers to as authoritative one sentence later). The actual fix for the coupling-drift class is procedural, not structural: this bullet and the headline above must be edited TOGETHER, in the same commit, whenever either is touched — which is what task #1161's own WARNING mechanism (below) already partially automates for the floor-vs-headline half of the coupling, and what this task's edit does by hand for the headline-vs-Next-trigger half, since the WARNING only compares the SCRIPT's floor against the SCRIPT's count — it cannot see into this Markdown file at all, so it structurally cannot detect two numbers disagreeing inside the doc.
