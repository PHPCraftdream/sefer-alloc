...
87. **[T, new static guard — record what it does and does NOT cover] `scripts/verify-ci-sentinels.mjs` (task #1150) statically checks every `grep -F "test <NAME> ... ok"` postcondition in `.github/workflows/ci.yml` against the real `#[should_panic]`/`#[ignore]` attributes on the `fn` it names — closing the class task #1146 (commit `77efd3a`) found and fixed by hand, but the guard has real, documented blind spots that a round-start reader must know before treating a green run as exhaustive.** (Filed 2026-08-19, task #1150.)
    ...
    - **Current-number-or-verdict:** 30 real sentinels checked (`grep -c 'grep -F' .github/workflows/ci.yml` = 33, of which 3 are comments mentioning `grep -F` rather than real postcondition lines — independently re-derived, NOT 32/2 as an earlier draft of this task's own brief stated); 24 unique test names; 6 in the `#[should_panic]` form (all six task #1146 fixed), 24 plain, 0 currently `#[ignore]`d among the 24 sentinel-tracked names (a broader repo-wide sweep independently found 11 literal `#[ignore]` MENTIONS under `tests/`, but every one is inside a `///`/`//!` doc comment describing past `#[ignore]` history, not a live `#[ignore]` attribute line — confirmed via `grep -rnE '^\s*#\[ignore\]' tests/`, zero matches). Counterfactual re-verified at filing: adding `#[should_panic]` to `crates/aligned-vmem/tests/mock.rs`'s `records_reserve_and_decommit` (a test whose sentinel is checked from 4 separate CI jobs/steps) makes the script report all 4 mismatches and exit 1; removing the attribute again restores exit 0 with 30/31 sentinels matching.
    ...
    - **Next trigger:** if a new CI job introduces a `grep -F "test ... ok"` sentinel shape this guard's parser does not recognize (e.g. a step structure other than `- run: <cmd>` / `- run: |` / `- name: X` + `run: |`, all three of which are handled today), the script's own malformed-sentinel path already fails loudly rather than silently skipping — but a genuinely new STEP SHAPE the parser's `splitIntoRunSteps` regex does not match at all would silently produce zero sentinels found for that step, which is why `verify-ci-sentinels.mjs`'s own checked-count is printed on every run (`node scripts/verify-ci-sentinels.mjs` prints "N sentinel(s) checked") — a round noticing the printed N drop unexpectedly below 30 without a corresponding sentinel removal in ci.yml is the signal to re-examine the parser.
    - **Evidence:** `scripts/verify-ci-sentinels.mjs` (script + its own header, which restates this does/does-NOT-cover split); `scripts/check-all.mjs` step 48 (appended at the array's end, `node scripts/verify-ci-sentinels.mjs`); the `check-matrix` job's last step in `.github/workflows/ci.yml`; re-derived census commands: `grep -c 'grep -F' .github/workflows/ci.yml` (33), `grep -n 'grep -F' .github/workflows/ci.yml` (30 real / 3 comments at lines 224, 1783, 1846 in the pre-task tree), `grep -rnE '^\s*#\[ignore\]' tests/` (0 matches); counterfactual commands run at filing (temporarily editing `crates/aligned-vmem/tests/mock.rs`, restored byte-identical per `git diff --stat` showing no change afterward).
