...
87. **[T, new static guard — record what it does and does NOT cover] `scripts/verify-ci-sentinels.mjs` (task #1150) statically checks every `grep -F "test <NAME> ... ok"` postcondition in `.github/workflows/ci.yml` against the real `#[should_panic]`/`#[ignore]` attributes on the `fn` it names — closing the class task #1146 (commit `77efd3a`) found and fixed by hand, but the guard has real, documented blind spots that a round-start reader must know before treating a green run as exhaustive.** (Filed 2026-08-19, task #1150.)
    ...
    - **Current-number-or-verdict (re-derived 2026-08-19, task #1155 — see the RE-DERIVE note below before trusting these figures on a later date):** the guard itself reports **33 sentinel(s) checked** (`node scripts/verify-ci-sentinels.mjs` → "OK — 33 sentinel(s) checked against their real `fn` attributes, all match."). Independently cross-checked against ci.yml's raw text: `grep -c 'grep -F' .github/workflows/ci.yml` = **40**, of which **7** are comment lines mentioning `grep -F` rather than real postcondition lines (`grep -n 'grep -F' .github/workflows/ci.yml | grep -cE '^[0-9]+:\s*#'`), leaving **33** real sentinel lines (40 − 7 = 33, matching the guard's own count) — currently at ci.yml lines 224, 244, 259, 389, 734, 1958, 2021 for the 7 comments. Of the 33 real sentinel lines: **27 unique test names** (dedup across all 33 lines: some names like `records_reserve_and_decommit` / `reentrancy_is_silently_dropped_with_outer_record_intact` are each checked from 4 separate CI steps), **6** in the `#[should_panic]` form (all six task #1146 fixed; 6 unique should_panic names, all appearing exactly once), and **27** in the plain form by RAW LINE COUNT (33 total − 6 should_panic = 27) — note this "27" is a different metric from the "27 unique names" figure and the two coincide numerically by chance, not by identity (unique PLAIN names alone = 21; 21 unique plain + 6 unique should_panic = 27 unique total, separately confirmed equal to the raw-plain-line count of 27). 0 currently `#[ignore]`d among the sentinel-tracked names (a broader repo-wide sweep independently found 11 literal `#[ignore]` MENTIONS under `tests/`, but every one is inside a `///`/`//!` doc comment describing past `#[ignore]` history, not a live `#[ignore]` attribute line — confirmed via `grep -rnE '^\s*#\[ignore\]' tests/`, zero matches, unchanged from filing). Counterfactual re-verified at filing (not re-run in this pass): adding `#[should_panic]` to `crates/aligned-vmem/tests/mock.rs`'s `records_reserve_and_decommit` (a test whose sentinel is checked from 4 separate CI jobs/steps) makes the script report all 4 mismatches and exit 1; removing the attribute again restores exit 0.
      - **Prior-revision record correction:** this card originally read "30 real sentinels ... 24 unique test names ... 24 plain" (task #1150 filing). Commit `4356e23`'s own body claims its landing text corrected "30 sentinel" to "31" alongside the `item 85→87` and `step 46→48` renumbering — but a direct re-read of that commit's actual diff hunk shows the "item 85→87" and "step 46→48" substitutions landed while the "30→31" substitution did NOT: the card text `4356e23` itself committed still reads "30 real sentinels ... 30/31 sentinels matching" verbatim. No commit between `4356e23` and the round-start SHA re-touched this card (`git log 4356e23..HEAD -- docs/CORRECTNESS_OPEN_ITEMS.md` is empty), so the 30/24/24 figures had been stale since filing, not merely uncorrected once. The count has also genuinely grown since (33 real sentinels today vs. 30/31 at various past filings), consistent with ci.yml gaining rows across intervening tasks — this is not solely a transcription-fix, the underlying file changed too.
      - **RE-DERIVE, do not carry forward:** every number in this bullet changes the moment a new `grep -F "test <NAME> ... ok"` row is added to or removed from ci.yml. A future reader MUST re-run `node scripts/verify-ci-sentinels.mjs` (for the checked count) plus the `grep -c`/`grep -n`/dedup commands cited above (for the comment/unique/should_panic/plain breakdown) rather than copy this bullet's numbers into a new commit or report — this exact figure went stale twice (30→31 claimed-but-not-landed, then 31→33 by further ci.yml growth) before anyone re-derived it against the live tree.
