...
59a. **[T] Linux half of item 59 — CLOSED for dispatch, OPEN for the kernel-response question (task #1160/F5).**

    The Linux `MAP_HUGETLB` success branch now DOES execute in CI: the `aligned-vmem-hugetlb-real` job (`.github/workflows/ci.yml`, added by tasks #1151/#1152) configures a real `nr_hugepages=64` pool and hard-asserts, via a path-activation oracle (`ci_hugetlb_real_pool_oracle_refuses_ordinary_page_fallback`), that `reserve_aligned_huge` actually received a real `MAP_HUGETLB` grant rather than silently falling back to ordinary pages. That job also runs the huge-decommit tests (`decommit_capability.rs`, `reservation_decommit_contract.rs`) under that real grant, proving the eligible-range decommit dispatch REACHES the real `madvise(2)`/`MADV_DONTNEED` backend call. **Caveat, precisely stated (task #1160/F1, the reason this is not simply CLOSED):** the kernel's own response to that `madvise` call is never observed — `libc_madvise` (`src/os/unix.rs`) discards the syscall's return value by design (task #719), and no test on the huge-page path reads it back. So the *dispatch* to the real backend under a real grant is execution-verified; the *kernel actually returning physical backing* on that call is not — it remains reasoned from the `madvise(2)` man page.
    - **Status:** PARTIALLY CLOSED — dispatch/grant proven; kernel-response proof remains open.
    - **Current-number-or-verdict (task #1160/F5, 2026-08-19):** `aligned-vmem-hugetlb-real` job is green and its 5 sentinel tests (`reserve_aligned_huge_accepts_huge_page_aligned_request`, `huge_decommit_attempts_increments_on_huge_reservation`, `huge_aligned_range_takes_the_real_backend_path_not_the_skip_path`, `method_try_decommit_huge_skip_returns_ok_and_counts`, `ci_hugetlb_real_pool_oracle_refuses_ordinary_page_fallback`) all pass under a real `MAP_HUGETLB` grant. No test asserts on `unix_madvise_successes()`/`unix_madvise_attempts()` (the counters that DO observe the kernel's return value, per `bench_internals/unix.rs`) on this path — that oracle exists (used by `macos_decommit_madvise_syscall_actually_succeeds` in `tests/smoke.rs`, Darwin-only) but is not wired into the hugetlb job.
    - **Next trigger:** wire a `unix_madvise_successes() == unix_madvise_attempts() > 0` assertion into a Linux hugetlb-pool test (mirroring `macos_decommit_madvise_syscall_actually_succeeds`'s pattern) to close the kernel-response gap. The job already runs with `bench-internals` enabled, so the counters are already live under it — only the assertion is missing.
    - **Evidence:** `.github/workflows/ci.yml` `aligned-vmem-hugetlb-real` job (lines ~351-432); `crates/aligned-vmem/tests/decommit_capability.rs` `huge_aligned_range_takes_the_real_backend_path_not_the_skip_path` and `ci_hugetlb_real_pool_oracle_refuses_ordinary_page_fallback`; `crates/aligned-vmem/src/os/unix.rs` `libc_madvise` (return-value discard, task #719) and its `bench-internals` `UNIX_MADVISE_ATTEMPTS`/`UNIX_MADVISE_SUCCESSES` counters; `crates/aligned-vmem/tests/smoke.rs` `macos_decommit_madvise_syscall_actually_succeeds` (the existing oracle pattern, Darwin-only today).
...
87. **[T, new static guard — record what it does and does NOT cover] `scripts/verify-ci-sentinels.mjs` (task #1150) statically checks every `grep -F "test <NAME> ... ok"` postcondition in `.github/workflows/ci.yml` against the real `#[should_panic]`/`#[ignore]` attributes on the `fn` it names — closing the class task #1146 (commit `77efd3a`) found and fixed by hand, but the guard has real, documented blind spots that a round-start reader must know before treating a green run as exhaustive.** (Filed 2026-08-19, task #1150.)
    ...
    - **Current-number-or-verdict (re-derived 2026-08-19, task #1161 — see the RE-DERIVE note below before trusting these figures on a later date):** the guard itself reports **37 sentinel(s) checked** (`node scripts/verify-ci-sentinels.mjs` → "OK — 37 sentinel(s) checked against their real `fn` attributes, all match."). Independently cross-checked against ci.yml's raw text: `grep -c 'grep -F' .github/workflows/ci.yml` = **44**, of which **7** are comment lines mentioning `grep -F` rather than real postcondition lines (`grep -n 'grep -F' .github/workflows/ci.yml | grep -cE '^[0-9]+:\s*#'`), leaving **37** real sentinel lines (44 − 7 = 37, matching the guard's own count) — currently at ci.yml lines 224, 244, 259, 411, 769, 1993, 2056 for the 7 comments (these line numbers moved from the previous re-derivation's 224/244/259/389/734/1958/2021 purely because ci.yml grew between commits `d85607c` and `dad4d7a` — same seven comments, shifted position, not a different set). Of the 37 real sentinel lines: **31 unique test names** (dedup across all 37 lines: `records_reserve_and_decommit` / `reentrancy_is_silently_dropped_with_outer_record_intact` are each checked from 4 separate CI steps — the only two repeats; every other name appears exactly once), **6** in the `#[should_panic]` form (all six task #1146 fixed; 6 unique should_panic names, all appearing exactly once), and **31** in the plain form by RAW LINE COUNT (37 total − 6 should_panic = 31) — this "31" (raw plain lines) coincides numerically with "31" (unique total names) by chance, not identity: unique PLAIN names alone = **25** (25 unique plain + 6 unique should_panic = 31 unique total, independently confirmed by a Node.js dedup pass over the parsed sentinel bodies, not just the shell pipeline). 0 currently `#[ignore]`d among the sentinel-tracked names (`grep -rnE '^\s*#\[ignore\]' tests/ crates/*/tests/`, zero matches, unchanged from filing). Counterfactual re-verified at filing (not re-run in this pass): adding `#[should_panic]` to `crates/aligned-vmem/tests/mock.rs`'s `records_reserve_and_decommit` (a test whose sentinel is checked from 4 separate CI jobs/steps) makes the script report all 4 mismatches and exit 1; removing the attribute again restores exit 0.
    ...
    - **Evidence:** `scripts/verify-ci-sentinels.mjs` (script + its own header, which restates this does/does-NOT-cover split, plus the task #1161 "F-drift" WARNING block); `scripts/check-all.mjs` (wired as its last array step, `node scripts/verify-ci-sentinels.mjs` — cite by content/name, not a bare step number, per this file's own step-numbering-decay lesson elsewhere in this index); the `check-matrix` job's last step in `.github/workflows/ci.yml`; re-derived census commands (all re-run 2026-08-19, task #1161, against the tree at commit `39279b5` + this task's uncommitted changes): `node scripts/verify-ci-sentinels.mjs` (prints "37 sentinel(s) checked"), `grep -c 'grep -F' .github/workflows/ci.yml` (44), `grep -n 'grep -F' .github/workflows/ci.yml | grep -cE '^[0-9]+:\s*#'` (7 comments, at lines 224/244/259/411/769/1993/2056 in the current tree), `grep -n 'grep -F' .github/workflows/ci.yml | grep -vE '^[0-9]+:\s*#' | wc -l` (37 real sentinel lines), `grep -rnE '^\s*#\[ignore\]' tests/ crates/*/tests/` (0 matches); a standalone Node.js dedup pass over the parsed sentinel bodies (31 unique names total, 6 unique should_panic, 25 unique plain — independent of the shell `sort -u` pipeline, to rule out a shell-locale sort artifact). The task-#1155 evidence line cited "33 real / 7 comments at lines 224/244/259/389/734/1958/2021" — independently re-verified THIS task against `git show d85607c:.github/workflows/ci.yml` and confirmed CORRECT at that commit (not a task-#1155 error); the drift to 37/44/7-shifted-lines is a `dad4d7a` (task #1152) regression that landed two commits after `d85607c`, not a re-derivation mistake. The pre-task-#1155 evidence line cited "30 real / 3 comments at lines 224, 1783, 1846 in the pre-task tree" — those line numbers do not resolve to comment lines in the current tree either; both older citations are superseded by the re-derived set above, not reconciled line-by-line.
