...
82. **[T, MEDIUM — census defect with a live coverage residue] Commit `826f150`'s 56-script census (task #1131) classified script coverage by "invoked by ANY gate" and concluded "a one-off AMONG GUARDS" — but under the coverage standard that same commit itself established and published in item 80, EIGHT local-gate step scripts (not zero, and not the seven the review that flagged this named) had no CI coverage at all; the census also claimed a record it had not made, and its three buckets do not sum to its own denominator of 56. Its substantive conclusion survives; the conclusion that drove the decision does not.** (Filed 2026-08-19, task #1135 record half. Every number below re-measured at commit `db9444f` — the worktree was clean, so worktree == `db9444f`, and the load-bearing greps were re-pinned to `git show db9444f:...` objects so concurrent same-wave ci.yml/check-all.mjs edits cannot shift the citation — not taken from the review's text, which itself proved wrong on two counts, both recorded below.)
    ...
    - **Status:** OPEN on sub-card (a)'s seven-script residue (a live CI-coverage gap; concurrent task #1136 is wiring ci.yml rows for scripts on the list, decided per script by measurement); `aligned-vmem-semver-check-optional.mjs` is CLOSED as covered-by-equivalent (task #1137/F3, `ci.yml:289`); sub-cards (b), (c) and (d) CLOSED on filing by this card. The card is written correct either way the wiring lands: the gap list is stated as measured at `db9444f`, and the Next trigger points at whatever remains uncovered after the wiring task lands.
    ...
    - **Next trigger:** after concurrent task #1136 (the wiring half of this same finding) lands, re-run the per-script row check (`git show <landing-sha>:.github/workflows/ci.yml | grep -E "run: .*scripts/"`) against the **seven-script** residue named in sub-card (a) — `argv-roundtrip-test.mjs`, `verify-internals-negative-boundary.mjs`, `verify-alloc-core-dbg-internals-exhaustive.mjs`, `vmem-doc-drift-guard.mjs`, `verify-aligned-vmem-bench-internals-exhaustive.mjs`, `stale-artifact-diagnosis.mjs`, `verify-vmem-page-constant-call-sites.mjs`. `aligned-vmem-semver-check-optional.mjs` is EXCLUDED from this trigger (see the correction above: it is covered-by-equivalent via `ci.yml:289`'s unconditional `cargo semver-checks check-release --package aligned-vmem` step, not an open gap) — a future wiring task adding a redundant `cargo semver-checks` invocation under that script's own name would not close anything real and is NOT what this card asks for. Every one of the seven still lacking a non-comment ci.yml run-row (or a demonstrated covered-by-equivalent case, per the same standard applied to the semver script) is this card's open residual, next-in-line for any future ci.yml edit per item 76's add-or-re-defer-explicitly convention — a round that edits ci.yml's `check-matrix` job must either add the missing row (or establish the equivalence) or re-defer it here explicitly. If all seven are resolved, this card closes with the wiring commit cited, and sub-cards (b)/(c) plus the "what survives" block remain as the permanent census correction.
