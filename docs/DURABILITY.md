...
|---|---|---|---|---|---|---|
| `RemoteFreeRing::head` / `tail` | `src/alloc_core/segment/remote_free_ring/mod.rs:772`,`781` | `u32` (per segment) | monotonic wrapping | **YES** — 2^32 cross-thread frees on one hot long-lived segment | wrap-safe by design: occupancy `tail.wrapping_sub(head)`, index `i % RING_CAP`, `RING_CAP` power-of-two so `2^32 % RING_CAP == 0` (continuous across wrap) | `tests/regression_ring_cursor_wrap.rs` (W7b) + const-assert `remote_free_ring/mod.rs:434` |
| `RemoteFreeRing::overflow` | `src/alloc_core/segment/remote_free_ring/mod.rs:787` | `u32` (per segment) | wrapping (diagnostic) | 2^32 overflow events on one segment | not correctness-load-bearing (diagnostic only; a wrap loses an overflow *count*, never a block) | `tests/regression_ring_overflow_counter.rs` |
| `DBG_RING_OVERFLOW` | `src/alloc_core/segment/remote_free_ring/mod.rs:377` | `AtomicU64` (process-wide) | wrapping (diagnostic) | 2^64 — unreachable | not correctness-load-bearing | `tests/regression_ring_overflow_counter.rs` |
...
| `TaggedIndex` tag (`free_slots` ABA) | `crates/tagged-index-stack/src/imp.rs:124` (`TaggedIndex<INDEX_BITS>`; the registry pins `INDEX_BITS=16` via `StackHead<16>`) | 48-bit tag in a `u64` (was 32) | strictly monotonic, saturating/sealed at `TAG_MAX` | 2^48 ≈ 2.8×10¹⁴ successful pushes ≈ **89 years @ 100k pushes/s** — a lifetime budget until seal, not a wrap: the tag bumps only on a successful push; pop preserves the observed tag (including the H-2 empty-transition case) and never bumps it; at `TAG_MAX` the next push returns `Err(TagExhausted)` — production never wraps the tag, and the head becomes permanently sealed against further pushes (pops keep draining the existing chain) | **repacked (W7a)** `index:16 | tag:48`; `MAX_HEAPS=4096` fits 16 bits with the `0xFFFF` empty sentinel above it; the non-recurring tag eliminates ABA outright over the head's available push lifetime (not merely mitigates it) | `crates/tagged-index-stack/tests/stack_unit.rs` (tag-wrap counterfactual, folded in from the retired crate-side `tests/regression_counter_wrap.rs`: `empty_sentinel_never_collides_with_a_live_index` + `empty_word_with_running_tag_reads_empty_through_tag_max`) + const-assert `TaggedIndex::_CHECK_BITS` (`crates/tagged-index-stack/src/imp.rs:149`, `INDEX_BITS` in `1..=16`) |
...
| `os::SEGMENTS_RESERVED_TOTAL` / `RELEASED_TOTAL` | `src/alloc_core/platform/os.rs:52`,`57` | `AtomicU64` | monotonic (`fetch_add`) | 2^64 — unreachable | bounded-by-width (diagnostic; net = reserved − released) | soak tests, `regression_*_no_leak.rs` |
