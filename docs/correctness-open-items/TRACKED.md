...
92. **[T, coverage gap created BY a red-`main` fix — filed in the same commit that creates it, per the item-89/90 class lesson] `DecommitOutcome::Refused` has NO test coverage on Unix, on macOS, or under miri: its only test is now Windows-only, because the mechanism it used to provoke an OS refusal is non-deterministic OUTSIDE Windows by construction.** (Filed 2026-08-20, task #1199.)
    ...
    - **Status:** OPEN.
    - **Current-number-or-verdict:** `Refused` is exercised on Windows only, by `refused_variant_is_produced_by_a_genuine_os_refusal` (`crates/aligned-vmem/tests/decommit_outcome.rs`, gated `#[cfg(all(windows, not(miri), not(aligned_vmem_mock)))]`). Every other row — Linux, macOS, the BSDs, miri, the mock backend — has zero executable assertion that a backend refusal surfaces as `Refused` rather than `Advised`. Re-derive with `grep -rn "Refused" crates/aligned-vmem/tests/`: hits only in `decommit_outcome.rs`, plus one comment in `try_decommit.rs`.
    - **Next trigger:** the first CI run in which the `aligned-vmem-hugetlb-real` job (or any future Linux job) is extended to exercise `dispatch_try_decommit`'s error path — that is the point at which the deterministic mechanism below must exist. Failing that: any edit to `crates/aligned-vmem/src/api/decommit.rs`'s `Err(e) => DecommitOutcome::Refused(e)` arm, which no test outside Windows can currently contradict.
