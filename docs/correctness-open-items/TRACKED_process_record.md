...
78. **[T, record correction — lint half, closed on filing] FOURTEEN R30-12 commit-prefix mis-slots recorded — thirteen grandfathered in `scripts/verify-commit-prefixes.mjs`, one same-wave inconsistency; history is not rewritten, so the correction lives here (sub-cards 1–3 filed by task #1117; sub-cards 4–5 added by task #1123, which found the task-#1117 card had never been updated when the fourth suppression entry appeared, and that `fb7dac8` — the one commit already in `origin/main`, hence the one with the strongest claim to a durable record — appeared in NO index at all; sub-card 6 added by task #1238 — the first UNPUSHED entry, a heuristic false-positive exemption rather than a genuine mis-slot on landed history, grandfathered with the owner's reword option explicitly left open; sub-card 7 added by task #1335 — a second heuristic false-positive exemption, `e25ec74`, whose only non-comment `src/`-adjacent delta is a `Cargo.toml` `description =` metadata string, not shipping code; sub-card 8 added by task #1505 — a third instance of the SAME `e25ec74` heuristic-blind-spot class, `eaa3310`, found by an independent `oxx` prepublish review of `size-classes` that also caught that the current `origin/main` landing SHA `74b49c8` is CI-red for an unrelated reason, `clippy::large_const_arrays` in `crates/size-classes/tests/proptest_builder.rs` — already fixed locally (`41149ed`) but never pushed; not filed as a separate open item since the fix already exists and only needs a push + green-CI confirmation, not further engineering work; sub-card 9 added by task #1514 — a fourth instance of the same Cargo.toml-`description` class, `abf2061`; sub-card 10 added by task #1514 — the FIRST genuine mis-slot in this card's own remediation wave, `66f47d2`, a real `Display::fmt` string change committed under a `docs(...)` prefix, deliberately grandfathered rather than rebased since it is unpushed, string-only, and pre-first-publish; sub-card 11 added by task #1584 — a second genuine mis-slot, `4c332ab`, a size-classes round-3 review commit whose `docs(size-classes)` prefix hid a real struct-field-removal change bundled with three comment-only fixes; PUSHED and un-amendable, unlike sub-card 10; sub-card 12 added by task #1678 — a fifth instance of the same Cargo.toml-`description` class, `08d8260`, this time on `tagged-index-stack` rather than `size-classes`; sub-card 13 added during the tagged-index-stack Sol-codex run-3 remediation wave — a sixth instance of the same Cargo.toml-`description` class, `ab6a2ed` (the P1-2 ABA-wording honesty commit); sub-card 14 added the same wave — a third genuine mis-slot, `67ea38d` (a real `perf(runtime):`-class push_index ordering change committed under a bare `perf(tis):` prefix), deliberately grandfathered rather than rebased: UNPUSHED but sitting inside a run of interleaved parallel-worktree merge commits, where a `--rebase-merges` reword risked re-resolving hand-verified merge conflicts from scratch (confirmed by a live abandoned attempt) for a one-word prefix fix — the same "reserved for explicit owner request" reasoning sub-card 10 already established for a lower-stakes case).** (Filed 2026-08-19; this card records ONLY the lint/record half — the remediation half belongs to a different agent's scope.)
    ...
    - **Current-number-or-verdict: six mis-slots, each premise independently re-verified for this card, not taken on the filing task's word:**
      ...
      8. **`eaa3310`, `docs(size-classes):` — but the commit touches `crates/size-classes/Cargo.toml`, outside the docs/examples/benches/tests/scripts allowlist.** Re-verified (task #1505, unpushed range, found by an independent `oxx` prepublish review of `size-classes`): `git show eaa3310 -- crates/size-classes/Cargo.toml` shows a ONE-LINE diff, the `description = "..."` key-value string, reworded from a changelog-fragment-shaped parenthetical to a self-contained sentence — the exact same defect class as sub-card 7 (`e25ec74`): crates.io-facing PACKAGE METADATA PROSE, not shipping/opt-in code, and `Cargo.toml`'s `version` field is untouched. `docs(size-classes):` is the honest prefix. Not taught to the guard as a new exception path, for the same reason sub-card 7 gives (guard-cleverness risk outweighs one more grandfather line). Grandfathered with the same self-cleaning posture: if the owner rebases/rewords the commit, remove this entry and this sub-card stands as the record.
      9. **`abf2061`, `docs(size-classes):` — a fourth instance of the identical Cargo.toml-`description` class (sub-cards 7/8).** Task #1514 shortened `crates/size-classes/Cargo.toml`'s `description = "..."` for crates.io search-result truncation (oxx P4-3); `git show abf2061 -- crates/size-classes/Cargo.toml` is a ONE-LINE diff, same field, same reasoning as sub-card 8. This same task ALSO fixed the recurrence at its root: `.gitignore` (a DIFFERENT false-positive class hit in the same wave, `880e5ee`, see below) was added to the guard's `MEASUREMENT_ONLY_ROOT_FILES` set as a genuine, low-risk, whole-file exemption (unlike `description`, `.gitignore` has no other field that could hide real behavior, so teaching the guard this one is not the risky cleverness sub-card 7's reasoning warns against) — but `Cargo.toml`'s `description` field was deliberately NOT given the same treatment, for the same reason sub-cards 7/8 already state, so a fifth instance remains possible and will need its own grandfather entry if it recurs.
      10. **`66f47d2`, `docs(size-classes):` — but the commit's one non-comment `src/` line is a REAL `Display::fmt` format-string change.** The first entry in this list that is a GENUINE mis-slot, not a heuristic false positive. Re-verified (task #1514, unpushed range): `git show 66f47d2 -- crates/size-classes/src/lib.rs` filtered to non-`///` lines shows exactly one hunk, `InvalidAlign`'s `Display` message changing from `"class_for: align (...) must be..."` to `"try_class_for: align (...) must be..."` (task #1509's own fix, correcting the message to name the function that actually constructs the error). This is the same defect class as sub-cards 1/5 (`09f4d16`/`fb7dac8`: a real behavior change to a public type under a `docs(...)` prefix) — the honest prefix would have been a plain `fix(size-classes):`. TWO differences from sub-cards 1/5, both recorded: (1) `09f4d16`/`fb7dac8` were already LANDED and un-amendable; `66f47d2` is UNPUSHED, so a rebase to fix the prefix was genuinely available — deliberately not taken, since rewriting local history for one string-literal wording correction is exactly the class of git-history action this repo's own conventions reserve for explicit owner request, not something to do unilaterally while processing an unrelated review's findings. (2) `09f4d16`'s changed string was `VmemError`'s `Display` on an ALREADY-PUBLISHED crate, so the change was a real breaking change for real downstream consumers; `InvalidAlign` belongs to `size-classes`, which has NEVER been published (crates.io returns 404, task #660), so this specific string has no consumer to break yet. Grandfathered with the same self-cleaning posture as every other unpushed entry: if the owner rebases/rewords the commit (e.g. to `fix(size-classes):`), remove this entry and this sub-card stands as the record of the decision point.
      11. **`4c332ab`, `docs(size-classes):` — but the commit's `src/lib.rs` delta bundles three genuine comment-only fixes with one real internal-implementation change.** A second GENUINE mis-slot (task #1584, pushed range): `git show 4c332ab -- crates/size-classes/src/lib.rs | grep -E '^[+-]' | grep -vE '^(\+\+\+|---)' | grep -vE '^[+-]\s*//'` shows four non-comment hunks, all from the size-classes round-3 review's P3-5 finding — `small_max: usize` removed from the `SizeClasses` struct, the `Debug` impl's field call changed from `&self.small_max` to `&self.small_max()`, the `Self { ..., small_max, ... }` initializer removed from `build()`, and `small_max()`'s body changed from `self.small_max` to `self.table[N - 1]`. Same defect class as sub-cards 1/5/10 (a real behavior-preserving code change under a `docs(...)` prefix), not the Cargo.toml-`description` false-positive class (sub-cards 7–9): `small_max` was a fully redundant cached field once `060fa09` moved `class_for` off the `> small_max` comparison, so removing it changes internal storage but not any OBSERVABLE output (`small_max()` returns the identical value; `Debug`'s printed line is unchanged, confirmed by the pre-existing `debug_impl_prints_a_summary_not_the_raw_tables` test staying green after the change) — no speedup measured or claimed, so the honest prefix would have been `fix(perf)`, matching the `5df56d3` `PerClass` `#[repr(C)]` precedent CLAUDE.md itself names for that slot. Unlike sub-card 10, this commit is PUSHED (`git merge-base --is-ancestor 4c332ab origin/main` exits 0 at filing time) — un-amendable per R30-12's non-retroactive posture, the same class as sub-cards 1/5. Grandfathered, not amended.
      12. **`08d8260`, `docs(tagged-index-stack):` — a fifth instance of the identical Cargo.toml-`description` class (sub-cards 7–9), this time on `tagged-index-stack` rather than `size-classes`.** Task #1678 (part of the `tagged-index-stack` round-1 review-fix wave, r1-T9) shortened `crates/tagged-index-stack/Cargo.toml`'s `description = "..."` from 556 to 134 chars for the same crates.io search-result-truncation reason as sub-cards 8/9. Unlike those two, the commit's diff is NOT a single line: `git show 08d8260 -- crates/tagged-index-stack/src/lib.rs | grep -E '^[+-]' | grep -vE '^(\+\+\+|---)' | grep -vE '^[+-]\s*///'` is EMPTY (the entire `src/lib.rs` delta, a "# Layout note" doc section, is `///` comment lines) and `crates/tagged-index-stack/CHANGELOG.md` is already exempt via `MEASUREMENT_ONLY_ROOT_FILES`'s basename match — so `Cargo.toml`'s one-line `description =` change remains the SOLE non-comment, non-exempt hunk in the whole commit, same defect class and same reasoning as sub-cards 7–9 verbatim. `docs(tagged-index-stack):` is the honest prefix. Not taught to the guard as a new exception path, for the same guard-cleverness reason sub-card 7 gives. Grandfathered with the same self-cleaning posture: if the owner rebases/rewords the commit, remove this entry and this sub-card stands as the record.
    ...
    - **Evidence:** the `git show`/`--stat`/`--name-only` commands quoted in the sub-cards above, re-run 2026-08-19 on branch `vmem-p4-l` for sub-cards 4–5 (sub-cards 1–3 re-run on `vmem-p3-j` at filing); `git grep -n "c766951\|fb7dac8"` returning nothing (pre-this-card); `git merge-base --is-ancestor fb7dac8 origin/main`; `node scripts/verify-commit-prefixes.mjs a61c582..HEAD` (PASS with 3 direction-2 warnings, exit 0); CLAUDE.md's R30-12 rule ("Active rules" section). Sub-card 6 (task #1238, 2026-08-20, unpushed range): the per-file filter `git show 2f9d7b9 --unified=0 --format= -- crates/aligned-vmem/src/` piped through the other sub-cards' comment filter (three files one `///` pair each, `os/unix.rs` five `//` lines + exactly two string-continuation lines); and the pristine-script check — `git show db248bb^:scripts/verify-commit-prefixes.mjs` into a scratch file outside the repo (with a stub `lib.mjs` pinning `REPO_ROOT` to this checkout; the script itself byte-identical) — run on the same `@{u}..HEAD` range, printing the identical 12-commits/6-warnings/1-FAILURE output with exit 1. Sub-card 7 (task #1335, 2026-08-25, unpushed range): `git show --unified=0 --format= e25ec74 -- crates/numa-shim/Cargo.toml crates/numa-shim/src/lib.rs`, personally re-read line by line — every `src/lib.rs` hunk starts with `//!`/`///`; the sole non-comment hunk is `Cargo.toml`'s `description =` line. Sub-card 8 (task #1505, 2026-08-27, unpushed range): `git show eaa3310 -- crates/size-classes/Cargo.toml`, personally re-read; single-hunk, single-line diff, the `description =` value only. Sub-cards 9–10 (task #1514, 2026-08-27, unpushed range): `git show abf2061 -- crates/size-classes/Cargo.toml` (single-line `description =` diff, same as sub-card 8); `git show 66f47d2 -- crates/size-classes/src/lib.rs | grep -E '^[+-]' | grep -vE '^[+-]\s*///' | grep -vE '^(\+\+\+|---)'` (exactly one hunk, the `Display::fmt` format string); `curl -H "User-Agent: ..." https://crates.io/api/v1/crates/size-classes` returning 404 (re-confirming task #660's finding that size-classes has no published consumers, cited in sub-card 10's reasoning); `node scripts/verify-commit-prefixes.mjs 74b49c8..HEAD` before and after each fix (PASS, exit 0, final state 3 grandfathered entries newly added this task: `abf2061`, `66f47d2`, plus the `.gitignore`-class fix for `880e5ee` via a `MEASUREMENT_ONLY_ROOT_FILES` addition that needed no grandfather entry at all). Sub-card 11 (task #1584, 2026-08-28, pushed range): `git show 4c332ab -- crates/size-classes/src/lib.rs | grep -E '^[+-]' | grep -vE '^(\+\+\+|---)' | grep -vE '^[+-]\s*//'` (exactly four non-comment lines, all from the `small_max` field removal); `git merge-base --is-ancestor 4c332ab origin/main` (exit 0, confirming pushed/un-amendable); `node scripts/verify-commit-prefixes.mjs 2349cc6..HEAD` before (FAILED, exit 1, naming `4c332ab`) and after the grandfather entry (PASS, exit 0). Sub-card 12 (task #1678, 2026-08-30, unpushed range, tagged-index-stack round-1 review-fix wave): `git show 08d8260 -- crates/tagged-index-stack/Cargo.toml` (single-line `description =` diff, same class as sub-cards 8/9); `git show 08d8260 -- crates/tagged-index-stack/src/lib.rs | grep -E '^[+-]' | grep -vE '^(\+\+\+|---)' | grep -vE '^[+-]\s*///'` (empty — the entire `src/lib.rs` delta is `///` doc-comment lines); `npm run check`'s `verify-commit-prefixes` step before (FAILED, exit 1, naming `08d8260`) and after the grandfather entry (re-run pending as part of this same task, before push).
