...
- 150. **Flaky test: `tagged_index_stack_ab_runner_scratch_guard::build_check_success_leaves_no_scratch_root` failed during the inner Windows link step.** — **CLOSED** (2026-09-23). The captured LNK1104 operand was a `.rlib` path of 281 UTF-16 units; the test now relocates its fixture and guards the representative path below 260 units. The exact targeted test passed; full closure evidence is in `ARCHIVE.md` item 150.
