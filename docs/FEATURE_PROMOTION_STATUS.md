...
| `numa-aware-mock` | yes (`Cargo.toml:740`) | no | **deliberately-opt-in** — TEST-ONLY marker feature; the mock backend itself is now the build-time `--cfg numa_shim_mock` (task #1288) — the feature alone no longer activates any mock; tests using it additionally require the cfg. | `Cargo.toml:713–740`. |
