...
> `src/alloc_core/segment/segment_header/mod.rs::node_id`. **Skip** for patch releases
...
`crates/numa-shim/` and `src/alloc_core/platform/numa.rs` were not touched, so per the
