...
| [`src/registry/bootstrap/`](../src/registry/bootstrap/mod.rs) | The primordial-segment carve / SegmentTable bootstrap seam — raw-pointer footprint carving of the metadata region under the atomic single-writer bootstrap protocol (seam carried by `bootstrap/{mod,registry,ensure,overflow_sidecar}.rs`). | `alloc-global` |
...
| [`src/registry/heap_registry/`](../src/registry/heap_registry/mod.rs) | Global heap slot-table — the `*mut HeapCore` pointer handoff out of a slot, consulted by every cross-thread routing decision (seam carried by `heap_registry/{mod,claim,stack,counters}.rs`). | `alloc-global` |
...
|---|---|---|
| Unit tests | Construction, edge cases, invariants | `tests/*.rs` (278 files) |
