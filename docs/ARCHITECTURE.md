...
|---|---|---|
| Unit tests | Construction, edge cases, invariants | `tests/*.rs` (170 files, as of CRATE-P3) |
...
| loom | Cross-thread protocol correctness under bounded interleavings (15 models) | `tests/loom_deferred_large.rs`, `tests/loom_dirty_multi_segment.rs`, `tests/loom_dirty_publish.rs`, `tests/loom_epoch.rs`, `tests/loom_free_slots_aba.rs`, `tests/loom_heap_overflow.rs`, `tests/loom_heap_overflow_drain_guard.rs`, `tests/loom_magazine_ring_compose.rs`, `tests/loom_overflow_first_retry.rs`, `tests/loom_remote_ring.rs`, `tests/loom_remote_ring_drain_guard.rs`, `tests/loom_sharded.rs`, `tests/loom_thread_free.rs`, `tests/loom_xthread_protocol.rs`, plus the extracted `crates/racy-ptr-cell/tests/loom_racy_ptr_cell.rs` real-type suite (CRATE-P3, replacing the former `loom_bootstrap_cas`/`loom_chunk_cas`/`loom_fallback_init`/`loom_overflow_sidecar_cas` shadow models) |
