...
### Confined unsafe seams (current inventory)

| Module | Role | Feature gate |
|---|---|---|
| [`src/alloc_core/os.rs`](../src/alloc_core/os.rs) | OS aperture: `mmap`/`munmap`/`madvise` (Linux), `VirtualAlloc`/`VirtualFree` (Windows). Every `unsafe` block carries `// SAFETY:`. | `alloc-core` |
| [`src/alloc_core/node.rs`](../src/alloc_core/node.rs) | Intrusive free-list node read/write: the single place that reads/writes the `next` pointer inside a free block. | `alloc-core` |
| [`src/alloc_core/numa.rs`](../src/alloc_core/numa.rs) | NUMA syscalls: Linux `mbind(2)` via `syscall(2)`, Windows `VirtualAllocExNuma`. No-op on macOS / miri. | `numa-aware` |
| [`src/concurrent/hand.rs`](../src/concurrent/hand.rs) | Epoch-based per-slot atomics for the lock-free concurrent Handle tier (3b-II). | `experimental` |
| `src/byte/*` | Research-tier byte allocator (Phase 4). Superseded by the segment substrate for production use. | `byte` |

Outside these modules, `unsafe` is a hard compile error
(`#![deny(unsafe_code)]` when any of those features are active;
`#![forbid(unsafe_code)]` with only the default `std` feature).
