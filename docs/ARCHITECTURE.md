...
|---|---|---|
| Unit tests | Construction, edge cases, invariants | `tests/*.rs` (247 files, as of task #1080) |
...
| loom | Cross-thread protocol correctness under bounded interleavings | `tests/loom_class_aware_dirty.rs`, `tests/loom_deferred_large.rs`, `tests/loom_dirty_multi_segment.rs`, `tests/loom_dirty_publish.rs`, `tests/loom_epoch.rs`, `tests/loom_heap_overflow.rs`, `tests/loom_heap_overflow_drain_guard.rs`, `tests/loom_magazine_ring_compose.rs`, `tests/loom_overflow_first_retry.rs`, `tests/loom_remote_ring.rs`, `tests/loom_remote_ring_drain_guard.rs`, `tests/loom_sharded.rs`, `tests/loom_thread_free.rs`, `tests/loom_xthread_protocol.rs`, plus the extracted `crates/racy-ptr-cell/tests/loom_racy_ptr_cell.rs` real-type suite (CRATE-P3, replacing the former `loom_bootstrap_cas`/`loom_chunk_cas`/`loom_fallback_init`/`loom_overflow_sidecar_cas` shadow models), plus the extracted `crates/tagged-index-stack/tests/loom_aba.rs` real-type suite (CRATE-P7, replacing the former in-tree `loom_free_slots_aba` shadow model — `heap_registry.rs`'s `free_slots` now consumes the crate's `TaggedIndexStack`, so the crate suite IS the coverage for the shipping code). (A former extracted `ring-mpsc` crate shipped a parallel real-type suite, CRATE-P4, for the shipping in-tree `RemoteFreeRing`/`HeapOverflow` protocol shape — never wired in, since the in-tree swap onto that crate was investigated and found NO-GO; the crate was removed from the workspace, task #655, and the seven in-tree ring/dirty models below remain the real coverage of those protocols, as they always were.) `tests/no_stale_loom_files.rs` (R13-5, task #275) is a structural CI guard, not itself a loom suite: it fails if any `tests/loom_*.rs` file is not referenced by a `--test` token in any `.github/workflows/ci.yml` loom job — added after `loom_class_aware_dirty.rs` shipped (R12-7 stage 2 / task #258) without being wired into any job. |
