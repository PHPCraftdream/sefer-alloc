...
**Compile-fail fixture swap.** `tests/compile_fail/hook_token_unconstructible/` (E0061 omitted-witness + E0423 forged-witness) removed; `tests/compile_fail/hook_call_requires_unsafe/` added (three bare hook calls → three E0133s; asserted by root `tests/tagged_index_stack_compile_fail.rs`, which also cites the compile-PASS counterpart pinned by `vec_backed_storage_push_pop_round_trips` + `push_pop_through_dyn_storage` in `tests/custom_storage_impl.rs` — the hooks are a barrier to misuse, not to legitimate use).
