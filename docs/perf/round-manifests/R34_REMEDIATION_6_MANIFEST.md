...
|---|------------|---------------|---------------------|------|----------|
| 1 | `dc003c957b40baacaa147ff35e81884e27b0b1b4` | `fix` | numa: guard macOS platform stub with not(miri) | #635 | **fix — correctness + CI-coverage** (`crates/numa-shim/src/lib.rs` cfg fix + new `numa-shim-macos-miri` CI job; not-yet-reachable-in-CI compile-time cfg path, no `production` behavior change) |
...
| 7 | `ebe615dd53c982f6b0eb6b0dcdb15bd08bea81eb` | `docs` | vmem: fix three doc-accuracy overclaims (F1/F6/F10, task #640) | #640 | **docs-only** (`crates/aligned-vmem/Cargo.toml` + `src/lib.rs` doc comments; zero code-path change) |
| 8 | `b17ffabfdf68e37e9f7f255c46cad846745781c5` | `docs` | P7 — fix last surviving cross-region overclaim in sefer-region rustdoc | #641 | **docs-only + test** (`crates/sefer-region/src/lib.rs` doc fix + one new demonstration test in `tests/smoke.rs`) |
...
| 11 | `7e1020f0f57fa8523e0830216c0aaa3c9cf88028` | `docs` | vmem,numa: add [package.metadata.docs.rs] to fix empty-default docs render (F3, task #644) | #644 | **docs-only** (`crates/aligned-vmem/Cargo.toml` + `crates/numa-shim/Cargo.toml` metadata; zero `[features]` table change) |
...
| 13 | `4c059fab167e37905290ecb646eff5e87ff7b844` | `fix` | vmem: resolve broken doc links + narrow mock dead-code allow (F4/F7/F8/F9, task #646) | #646 | **fix — doc links + lint-suppression narrowing** (`crates/aligned-vmem/src/lib.rs`, `src/fault_injection.rs`, root `Cargo.toml` feature-alias migration; the dead-code-allow narrowing is a real hygiene fix, bundled with doc-only F4/F7/F9) |
...
has zero `cfg`-gated uses anywhere in `crates/aligned-vmem/src/` (it survives only
as a pure feature-name alias in `crates/aligned-vmem/Cargo.toml` for external
