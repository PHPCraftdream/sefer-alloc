...
57. **[L] Precedent, record-only (task #1088/L11): inverted R30-12 prefixes on `a828ebc` (bare `fix:`) vs `b98edb0` (`fix(perf)`) — the commit making the STRONGER claim got the WEAKER prefix, and no lint can catch this class without false positives.**

    - **Status:** RECORDED — no code change possible or needed. History is explicitly non-retroactive (CLAUDE.md R30-12: "no historical commit message is retagged... it governs new commits going forward only"); neither commit is amended, this card is the whole remediation.
