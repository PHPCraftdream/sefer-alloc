...
28. **R29 post-round readonly review — perf-report methodology findings,
    independently re-verified (R30-4, task #453). RESOLVED.**

   > **Current state**
   > - **Status:** RESOLVED — all 8 sub-findings (a)-(h) independently
   >   re-verified against cited raw logs/source in task #453 (R30-4).
   >   Source reviews: `docs/reviews/2026-07-29-r29-readonly-review.md` and
   >   `docs/reviews/2026-07-30-r29-followup-readonly-review.md` §2.2-§2.6
   >   (the followup review independently corroborated several of the same
   >   findings). 5 of 8 CONFIRMED (dated correction sections appended to
   >   the affected reports, append-only, originals preserved), 2 of 8
   >   PARTIAL (corrections appended with a narrower/corrected framing than
   >   either the original report or the review's paraphrase), 1 of 8
   >   REFUTED (recorded here, not corrected in the report it concerns —
   >   the original report's number stands as originally published).
   > - **Verdicts:**
   >   **(a) REFUTED** — R29-16's "21.4× Ir" (3,067 vs 65,624 Ir) is NOT a
   >   "Valgrind artifact." `Node::zero` (`src/alloc_core/node.rs:135-144`)
   >   is a plain `write_bytes` memset; Callgrind's VEX-IR translation
   >   unrolls REP-prefixed string instructions and charges `Ir` once per
   >   loop iteration, so ~1 Ir/byte on a 64 KiB memset is EXPECTED
   >   Callgrind behavior, corroborated by R29-16's own L1/L2/RAM
   >   cache-line-touching signature (recycled arm: 1,157 L2 hits / 2,135
   >   RAM hits vs. virgin arm's 111/1,097). No correction needed to the
   >   21.4× figure; a methodological Ir-vs-wall-clock caveat was appended
   >   to `R29_16_VIRGIN_ZERO_SKIP_CALLOC_GATE.md` §9 regardless, per this
   >   task's brief.
   >   **(b) CONFIRMED (narrower framing)** — R29-3 §5's "net loss" headline
   >   overstates against the report's OWN A′−B numbers: both saved runs
   >   show a same-direction positive saving (5.1%, 1.7%), dismissed as
   >   "noise" without any stated variance/stddev in the document. Corrected
   >   framing: **workload-sensitive / not currently a priority** (not
   >   "mathematically negative for all shapes," and not "a clear win"
   >   either) — a touch-density sweep is the precondition for reopening.
   >   **(c) CONFIRMED** — `examples/r29_3_decomposition_gate.rs` computes
   >   `total.elapsed() / N` (arithmetic mean) at lines 100/113/150-151/179
   >   and the report/example both label it "median of 200." Pure relabeling
   >   fix; underlying numbers unchanged; example source NOT touched.
   >   **(d) CONFIRMED** — Measurement B/A′ (`examples/r29_3_decomposition_gate.rs`
   >   lines 126-127/135-136/145-146/166-167/174-175) touch ALL ~1,006
   >   payload pages every cycle — full touch density. §6's unconditional
   >   "(4+5) dominates" verdict is now scoped explicitly to full-touch
   >   density; at sparse touch density the fixed `MADV_DONTNEED` cost
   >   could dominate differently and reverse the ordering — unmeasured,
   >   named as the reopening precondition.
   >   **(e) CONFIRMED** — R29-13's cited sha256
   >   (`d40e8280b433892e17605b9b96c28baaebf852a8f3d70057ba64cd47ac0ec98`) is
   >   **63 characters**, not the required 64 (verified by direct count). A
   >   reconstruction attempt from the report's own base SHA (`34f3702`)
   >   forward to the landing commit (`894e9e3`) produced a DIFFERENT hash
   >   (`1c4ea5e1...`), confirming the original was computed over an
   >   uncommitted working-tree state that is not reconstructable from git
   >   history. Filed as an honest, permanent irreproducibility gap — NOT
   >   fixed with a fabricated replacement hash. First real-world
   >   application of the R29-6 immutable-provenance rule; exposed that the
   >   rule's weakest-justified option (ad-hoc patch+file concatenation) was
   >   used for a tree state that was never preserved as a resolvable git
   >   object, independent of the transcription error.
   >   **(f) CONFIRMED** — R29-16 cites no base SHA / immutable source
   >   identity anywhere (grep for `Base revision|sha256|Measured on|commit
   >   SHA|main @` returns zero matches in the file). Per CLAUDE.md's
   >   explicit non-retroactive carve-out, NOT retroactively reconstructed —
   >   noted as a known, permanent provenance gap in that historical report;
   >   the operative successor (`R30_3_VIRGIN_ZERO_SKIP_NATIVE_GATE.md`,
   >   task #452) already carries proper provenance and supersedes R29-16
   >   for the promotion decision regardless (see item 25).
   >   **(g) PARTIAL** — two distinct issues in R29-13: (1) line 8's "32x
   >   the small pool's 16 MiB byte cap" is a plain arithmetic error
   >   (256/16 = 16, not 32) — corrected to 16×/8× (cap=4/cap=8 byte caps),
   >   exact. (2) line 344's "30x the small pool's proven ~8 MiB/heap
   >   retention" compares an ABSOLUTE quantity (238 MiB) to R27-3's
   >   cap8-minus-cap4 DELTA (~8 MiB), not an absolute retention floor —
   >   confirmed apples-to-oranges. No single unambiguous corrected ratio
   >   exists (R27-3 and R29-13 measure retention at different lifecycle
   >   points and against different baselines); two like-for-like absolute
   >   comparisons computed directly from each report's own §0 tables give
   >   **~7.6× (vs. small-pool cap=8 absolute, 31.23 MiB)** and **~10.3× (vs.
   >   cap=4 absolute, 23.23 MiB)** — both well below the retracted 30× but
   >   not offered as *the* replacement headline (a true comparison would
   >   need the small pool's own fully-drained floor, not measured here).
   >   The review's own candidate "~12×" was NOT independently reproducible
   >   from either report's cited numbers under any reading tried.
   >   **(h) PARTIAL** — both cited ratios in R29-5 are arithmetically exact
   >   (33/60,722 = 0.054%; 33/40 = 82.5%, confirmed by direct computation).
   >   Not an arithmetic error — a framing gap: R29-5's own §1 already
   >   discloses the 40-object population and that 33/40 promoted, but §0's
   >   headline table never computes or states 82.5%, and the "RARE"
   >   framing doesn't caveat that a promotion-heavy consumer workload could
   >   see a very different rate. Corrected framing added to R29-5 §9:
   >   promotion is a small fraction of THIS MIXED workload overall, but
   >   common (82.5%) among objects deliberately grown into the promotable
   >   region — this does NOT support rejecting Linux `mremap` for a
   >   promotion-heavy consumer workload. Item 6's "stays deferred"
   >   disposition is unchanged (this report's own §4 already disclaimed
   >   generalization to other workload shapes).
   > - **Next trigger:** none for the re-verification itself (closed). Two
   >   corrections (b/d, R29-3; h, R29-5) name unmeasured preconditions
   >   (touch-density sweep; promotion-heavy workload measurement) for
   >   reopening their respective deferred items — those preconditions are
   >   NOT triggered by this task and stay open only as already-named
   >   revisit triggers on items 15/16 and 6 respectively, not as new items.
   > - **Evidence:** `docs/reviews/2026-07-29-r29-readonly-review.md`;
   >   `docs/reviews/2026-07-30-r29-followup-readonly-review.md` §2.2-§2.6;
   >   dated correction sections appended (append-only) to
