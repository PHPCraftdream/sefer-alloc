...
30. **R14-5 — `large-cache-extended` CONDITIONAL-GO, not promoted; owner entry
   ...
   >   `src/alloc_core/config/large_cache_config.rs` already rejected this same
   ...
   >   comment (`src/alloc_core/config/profile.rs`), the `Profile` module doc's
   ...
   >   (`src/alloc_core/config/profile.rs`) — requires `large-cache-extended`
   ...
   > - **Files:** `src/alloc_core/config/profile.rs` (`LargeCachePolicy::DiverseTurnover`
...
31. **UNVERIFIED-BY-ME findings from the Round 30 full independent review
   ...
   >     shipped doc comments (`src/alloc_core/config/profile.rs:38-45,68-84`) and
   ...
   >   doc comments in `src/alloc_core/config/profile.rs`) is NOT applied by this
...
27. **R29-13 — large-cache `headroom_bytes` (default 256 MiB/heap) idle-RSS
    ...
    >   (`src/alloc_core/large/alloc_core_large.rs:127-194`, whole-`SEGMENT` rounding,
    ...
    >   `src/alloc_core/config/profile.rs:38-45` — flagged here as an input for
    ...
    >   (`src/alloc_core/config/profile.rs`) states the regime caveat explicitly:
...
39. **F13 (2026-08-03, task #505) — three areas checked and found thin/already-
    ...
    >     `src/alloc_core/platform/size_classes.rs:74` (`SMALL_ALIGN_MAX = 16`): any
    ...
    >     (833 lines) + `src/alloc_core/platform/numa.rs` (125 lines) exist as the
...
57. **[A] `aligned-vmem` third-audit perf candidates (P1/P3) — owners assigned,
    ...
    - **[Filed 2026-08-20, task #1214, from
      ...
      - **P3 (Windows unprivileged large-page cascade) is the SAME gap as
        `docs/correctness-open-items/TRACKED_platform_contracts.md` item 59b (Windows half of
...
61. **[D] `tagged-index-stack` store-elision candidate in the `push_index`
    ...
    - **Status:** OPEN — instrument-ready, measurement-only; no production
      change.
    - **Current-number-or-verdict:** `store_elided` changes only
      `push_index_impl`: x86 `62 → 71` (+14.5%), AArch64 default `78 → 77`
      (−1.3%), and AArch64 `+lse` `71 → 69` (−2.8%). The relevant static
      ...
      source HEAD `b6b33c5665567dd21106efa749f9d5a5303db57c`, tree
      `13b2b5f4848e05b020ef27ec51d29a3f589c474f`, source-input digest
      `0f16a1bf7f48da2d6dedb6e879532c74ddb036e4a5a7808cc875ad46c4528a4d`,
      artifact commit `e4ad8523f310b488c1f91b23a8129e577b28ea66`, and host receipt hash remains
    ...
    - **Evidence:** `docs/perf/TIS_LINK_ORDERING_WEAK_CAS_GATE.md`; source-input
      identity `b6b33c5665567dd21106efa749f9d5a5303db57c` / tree
      `13b2b5f4848e05b020ef27ec51d29a3f589c474f` / digest
      `0f16a1bf7f48da2d6dedb6e879532c74ddb036e4a5a7808cc875ad46c4528a4d`;
      current codegen artifact commit `e4ad8523f310b488c1f91b23a8129e577b28ea66`; and the historical ODB/HS
...
62. **[D] `tagged-index-stack` link-cell ordering (P3-1) wall-clock A/B.**
    ...
    - **Current-number-or-verdict:** authoritative codegen is source-input
      identical to source HEAD `b6b33c5665567dd21106efa749f9d5a5303db57c`,
      tree `13b2b5f4848e05b020ef27ec51d29a3f589c474f`, source-input digest
      `0f16a1bf7f48da2d6dedb6e879532c74ddb036e4a5a7808cc875ad46c4528a4d`,
      ...
      results. The current artifact commit is `e4ad8523f310b488c1f91b23a8129e577b28ea66`; the host receipt
      ...
      Matrix/static deltas are unchanged from artifact `0203577`; no ARM timing
      claim is inferred.
    ...
    - **Evidence:** `docs/perf/TIS_LINK_ORDERING_WEAK_CAS_GATE.md` and its two
      current codegen CSVs/raw logs/asm; source HEAD `b6b33c5665567dd21106efa749f9d5a5303db57c`,
      tree `13b2b5f4848e05b020ef27ec51d29a3f589c474f`, digest
      `0f16a1bf7f48da2d6dedb6e879532c74ddb036e4a5a7808cc875ad46c4528a4d`;
...
- **Task #1088/L11 precedent — inverted R30-12 prefixes on `a828ebc` (bare `fix:`) vs `b98edb0` (`fix(perf)`) — this index's own former item 57.** Closed 2026-08-18 (OH3 wave, readonly-review finding L5) — record-only by design: history is non-retroactive under R30-12, neither commit is amended, and the recorded card WAS the whole remediation (`Next trigger: none mechanical`), yet its `[L]` header kept presenting it as an open item at every round-start read — the exact structural defect CLAUDE.md's R34-24 rule names. Status updated to CLOSED, full precedent text (including the "why the lint cannot catch this class" reasoning — still the instrument to consult before writing any perf-family commit subject) archived at `docs/perf/OPEN_ITEMS_ARCHIVE.md` § "Recently resolved — full closure trail".
