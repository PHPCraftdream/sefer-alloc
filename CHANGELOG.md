...
- **[bench, P1] Task #665 (commit `67062b3`) — fixed a bench harness fidelity defect before the README perf table shipped.** `bench_scale_tool::Harness::bench_batched`'s per-iteration setup/routine split had been misused for the `insert`/`remove` workloads, inflating their measured cost; corrected to the harness's intended usage and added `st/churn`/`sync/churn`/`raw/churn` steady-state workloads alongside the existing cold-path ones, with the README table now labeling each row's measurement regime explicitly (e.g. "cold: fresh map, allocation + full teardown inside the timed window") so the numbers aren't read as more precise than they are.
...
- **[test, P1] Task #668 (commit `cec0333`) — closed two zero-coverage gaps: I5 (drop-once) and `clear()`'s happy path.** Neither invariant had a single test. Added 16 tests total covering drop-exactly-once across `remove`/`Region`-drop/a mixed insert-remove sequence (both `Region` and `SyncRegion`), `clear()`'s happy path, and assorted previously-uncovered methods (`iter_mut`, `get_mut`, `Default`, `with_capacity`, `reserve`). Personally injected a counterfactual (`std::mem::forget`) into the drop-count test and confirmed it fails before reverting.
