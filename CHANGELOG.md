...
- **[correctness fix, P1] R34-11 (task #530) — added a bounded catch-up loop (`DECAY_CATCHUP_MAX_STEPS = 8`) to `maybe_decay_large_cache`, closing R34-10's sparse-retention gap while preserving R32-8's original throughput benefit.** Re-gated: final gap reduced 3→1 segments; the sparse-regime interval-30 catch-up correctly fires 3 steps in one clock read; R32-8's throughput win preserved (~57-67% depending on run). Commits `73dceca` (fix) + `a100647` (gate) + `43115cf` (CSV self-correction).
