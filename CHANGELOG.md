...
### Round 34 — the round's three earlier tasks landed as commits without dedicated CHANGELOG bullets of their own: R34-1/task #520 and R34-2/task #521 (both `docs(process)` corrections, already inline-referenced under Round 33's R33-5 entry above) and R34-3/task #522 (`feat(api)` gating of this crate's internals-reaching `src/` surface behind a new `internals` Cargo feature, with CI/check-matrix/release gates synced to match). This section documents the round's substantive dependency-security deliverable.

**Runtime improvements this round: 0.** No shipping algorithm, production default, or opt-in code path's observable behavior changed; `production`'s feature composition is unchanged. R34-3's `internals` feature is a build-gating/visibility reorganization (no `src/` behavior change), not a runtime change.
