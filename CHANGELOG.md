...
- **[test, P3] Task #694 (commit `ea52f85`) — made `clear_partial_under_panic`'s assertions order-agnostic.** The test asserted `slotmap`'s clear/drain visited values in a SPECIFIC order (ids 0,1,2 dropped; 3,4 survived) — but `slotmap`'s own `iter()` doc states iteration order is unspecified, and `Cargo.toml` pins `slotmap = "1"` (floats across 1.x minors), so a future minor bump changing internal drain order would have made this test go red as a false regression even though the crate's real order-free contract still held. Rewrote both `Region` and `SyncRegion` scenarios to compute the survivor/dropped classification from `iter()`'s actual output (via a `HashSet` complement) rather than hardcoded positions, keeping the same total-accounting strength (`drop_count == 3`, `len() == 2`).
