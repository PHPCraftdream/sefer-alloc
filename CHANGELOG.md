...
#### `aligned-vmem` — round-3 follow-up (2026-08-12, tasks #858-864)

A fresh review of the post-#851–#857 tree (`docs/reviews/2026-08-12-aligned-vmem-round3-review.md`, findings F1-F11) found round-2 fix residue in the form of assertions and docs that were updated at call sites but not one line away, plus one open CI gap from round 2's own W14. All 7 tasks delegated to `/crush` with full personal zero-trust re-verification. **This is a sub-crate follow-up — `aligned-vmem` is a workspace member library not consumed by `sefer-alloc`'s own `production` feature bundle, so none of these changes affect the allocator's shipping runtime.**

- **[fix(vmem), ci, P1] Task #858 (commit `75bba05`) — F1: fixed `mock.rs` page-size assertion; F2: added Windows CI test step.** F1 (MEDIUM): `tests/mock.rs`'s `records_reserve_and_decommit` had its two `decommit` calls already fixed to `page_size()` (task #855/W9), but the assertion below them still compared the recorded offset against the compile-time `PAGE` constant — effectively asserting `page_size() == PAGE` (4096), false on any 16 KiB/64 KiB-page host. This was confirmed as the actual cause of the "test macos (production)" CI job failure on the task #855 landing SHA (GitHub-hosted macos-latest is Apple Silicon, `page_size() == 16384`). Fixed by replacing `start == PAGE` with `start == page_size()`. F2 (MEDIUM): `.github/workflows/ci.yml`'s `test-windows` job never ran `cargo test -p aligned-vmem` — task #848's Windows single-call fast path and its own regression test (`tests/lazy_commit.rs`) had never executed on the only platform where that code path exists. Added `cargo test -p aligned-vmem --all-features --no-fail-fast` to `test-windows`, mirroring the equivalent step task #856 added to `test-macos`.
- **[docs(vmem), P2] Task #859 (commit `fe19572`) — F3: synced root crate's stale Windows counter docs + added missing `dbg_*` forwarders.** Task #853 (round 2) correctly split `WINDOWS_RESERVE_COMMIT_CALLS` inside `crates/vmem/src/lib.rs`, but 4 sites outside that file described the stale, pre-split state: (1) `src/alloc_core/alloc_core_core_diag.rs`'s `dbg_windows_reserve_commit_calls` still said "Each call issues exactly 2 syscalls" — false since task #848, and this is the surface examples/r32_13_windows_reserve_commit_decomposition_gate.rs actually reads as its path-activation oracle, so a future perf round using it would compute syscalls as 2× calls (wrong by up to 2× for any single-call-path activity); (2) `dbg_reset_vmem_bench_internals_counters` said "reset all three" — corrected to four; (3) `crates/vmem/Cargo.toml`'s `bench-internals` feature doc still described "2 syscalls each" and said the statics mirror sefer-alloc's "`#[doc(hidden)]` accessors" convention — task #853 hid the STATICS (correct) and left the accessor FUNCTIONS public (also correct, intentional) — reworded to "`#[doc(hidden)]` statics"; (4) root `Cargo.toml` (two comment blocks) named `WINDOWS_RESERVE_COMMIT_CALLS`, a static that no longer exists — updated to name both split statics. Substantive fix: added `dbg_windows_reserve_commit_single_calls()` and `dbg_windows_reserve_commit_two_call_pairs()` forwarders in `alloc_core_core_diag.rs` beside the existing one, matching its exact style (`#[doc(hidden)]`, `bench-internals`-gated, `#[must_use]`) — the split counters added by task #853 were previously unreachable from the root crate that needs them for future perf gates.
- **[docs(vmem), P2] Task #860 (commit `fd032af`) — F4: fixed reserve_aligned doc drift; F5: fixed README API table entry.** F4 (LOW): `reserve_aligned`'s rustdoc said Windows "unconditionally over-reserves `size + align` bytes" — false since task #848 for `align <= 64 KiB` (single-call path, `base == region`, no over-reserve at all). Corrected to match the crate's own already-accurate module-level bench-internals comment. The same over-generalization was also in the crates.io package description (`Cargo.toml:7`) — this is the fourth documented drift of this one sentence (tasks #640, #650, #842, #854). Both now state the real conditional behavior. F5 (LOW): the README's API table gave `into_parts()` the wrong return type (`ReservationParts`) and label ("typed form") — `into_parts()` actually returns a 3-tuple (`(*mut u8, usize, usize)`); `ReservationParts` is what `into_reservation_parts()` returns (added by task #845/V8), which the table had no row for at all. Fixed the `into_parts` row to show its real tuple return type labeled "(legacy tuple form)", and added a new row for `into_reservation_parts() -> ReservationParts` labeled "(typed form)" — now agrees with the README's own runnable 3-tuple-destructuring example above it.
- **[docs(vmem), fix(vmem), P2] Task #861 (commit `22f1e55`) — F6: disclosed Windows retry-branch counter behavior; F7: guarded allocation-granularity constant.** F6 (LOW): `WINDOWS_RESERVE_COMMIT_SINGLE_CALLS`'s rustdoc promised "exactly 1 syscall" per increment, but the single-call path's huge-pages retry branch (large-page `VirtualAlloc` fails, falls back to a second plain `VirtualAlloc`) issues 2 syscalls under one increment. The sibling counter `WINDOWS_RESERVE_COMMIT_TWO_CALL_PAIRS` already discloses its own analogous retry case explicitly — added the matching disclosure here, same convention (retry not double-counted, now stated in the doc rather than silently assumed). F7 (LOW): the single-call fast path's alignment guarantee rests on the hardcoded `WIN_ALLOCATION_GRANULARITY = 65536` constant, never checked against the OS's own answer, even though the crate already queries it via the existing `GetSystemInfo` call in `query_os_page_size` (`dw_allocation_granularity` was declared but unread). Added a `debug_assert!` right after that existing `GetSystemInfo` call — verifies `dw_allocation_granularity >= WIN_ALLOCATION_GRANULARITY`, so a hypothetical future Windows target reporting a smaller value would be caught in a debug build instead of silently returning a misaligned base. Debug-only safety net, no dispatch-logic change — every currently-supported target reports exactly 65536.
- **[docs(vmem), P2] Task #862 (commit `91d5555`) — F8/F9/F10: three doc fixes from round-2's hand-resolved merges.** F8 (INFO): `is_huge`'s rustdoc had two artefacts from the hand-resolved doc conflicts in tasks #852/#853 — a sentence rendering as a stray list continuation instead of its own paragraph (fixed by de-indenting + adding a blank `///` line), a missing "falls back to ordinary pages" paragraph the sibling field doc already had (added), and literal escaped quotes `\"best-effort\"` inconsistent with the unescaped style used elsewhere in the same file (fixed). Also added a note that `from_raw_parts`-adopted reservations always report `is_huge() == false` regardless of actual origin. F9 (INFO): the Windows two-call path derives `granted_huge` from the requested flag rather than an observed grant (the Unix side already fixed this exact class of issue in task #852/W2 via `HUGE_SUPPORTED`). Added a doc note at the increment site explaining this is a documented-but-unenforced invariant (the branch is unreachable per the crate's own Win32-contract reasoning) — no runtime behavior changed. F10 (INFO): `mock::Call::Release`'s variant doc named only the manual `release()` path as a producer; task #845/V9 made `Drop` record the same variant too (confirmed by the existing `drop_records_release` test). Updated the doc to name both producers.
- **[docs, P1] Task #863 (commit `c14bd3a`) — F11: committed review docs + wrote the #851-857 CHANGELOG entry + fixed stale commit ref.** Committed the three previously-untracked review documents (`docs/reviews/2026-08-12-aligned-vmem-{code-quality,post-campaign-closing,round3}-review.md`) — every commit message across all three rounds cites these paths; the citations now resolve. Wrote the CHANGELOG entry `CHANGELOG.md`'s own text said was owed for tasks #851-857 (round 2, findings W1-W16 + P-A/P-B/P-C), matching the format of the existing #842-850 entry immediately above it. Includes an honest account of task #857's own mid-round fabrication discovery and correction, since that is itself part of what actually happened in this round. Fixed `docs/CORRECTNESS_OPEN_ITEMS.md:1837`'s stale "(commit pending)" reference for task #851's fix — now cites the real commit `78ecc81`.
- **[fix(numa), P1] Task #864 (commit `d66c3c7`) — numa-shim: fixed `reserve_on_node` test's stale over-reserve assumption, invalidated by aligned-vmem #848.** Found NOT via a review document but by watching CI go red on the aligned-vmem round-1+round-2 campaign's landing SHA (76ac08f..b20b4a5) after pushing to origin/main. `crates/numa/tests/smoke.rs`'s `reserve_on_node_commits_only_the_requested_span_not_the_whole_over_reservation` test used `align = page * 8` (32 KiB on a 4 KiB-page host), chosen to be "> size" to "guarantee real slack to probe." aligned-vmem's task #848 (Windows single-call fast path) changed the meaning of "guaranteed slack": for `align <= 64 KiB` (WIN_ALLOCATION_GRANULARITY) AND `commit_len == size`, `win_reserve_commit` now takes a single-call path returning `base == region` with ZERO over-reserve. At `align=32768` (under the 64 KiB threshold), this numa-shim test's own downstream call into aligned-vmem now reaches that fast path, and its hard assertion "tail slack must be non-empty by construction" panicked — observed live in CI. Fixed by raising `align` to 128 KiB (comfortably above the 64 KiB threshold and well below the 2 MiB huge-page granularity on either platform), forcing the genuinely over-reserving two-call path — this preserves the test's real purpose (verify the over-reserved slack is never committed) without weakening any assertion.

All 11 findings from the round-3 review are now closed. `aligned-vmem`'s own crates.io publish decision (task #658) remains deferred to an explicit maintainer go-ahead, gated behind the two prior rounds finishing.

#### `aligned-vmem` — round-4 follow-up (2026-08-13, tasks #867-874)

A fresh review of the post-#858–#864 tree (`docs/reviews/2026-08-12-aligned-vmem-round4-review.md`, findings R1-R13) found a real verification hole (both platform CI rows added by rounds 2/3 to cover the real OS backends actually ran the `mock` recording stub instead — proven by an executed counterfactual, not just read), plus more of the same "the fix introduces the next round's finding" residue this campaign keeps surfacing. All 8 tasks delegated to `/crush` with full personal zero-trust re-verification, followed by a dedicated round-4 CLOSING review (`docs/reviews/2026-08-12-aligned-vmem-round4-closing-review.md`, findings CR1-CR10) that caught issues in the remediation itself — CR1 (the CI fix deleted the `--all-features` step it was supposed to supplement, un-covering round-3's own F1 fix on the macOS/16 KiB-page runner it was written for) and CR2 (the new doc-drift guard passes clean on both real historical drift sentences it exists to catch, because its qualifier is an OR and every real drift sentence also says "align"/"Windows") are the two that mattered; CR3/CR5/CR6/CR8/CR4 (mis-citations and half-applied fixes) were corrected in this same pass. **This is a sub-crate follow-up — `aligned-vmem` is a workspace member library not consumed by `sefer-alloc`'s own `production` feature bundle, so none of these changes affect the allocator's shipping runtime.**

- **[ci, P0] Task #867 (commit `e0d921a`, closing-review CR1 fix folded in) — R1/R2: fixed the platform CI rows to exercise the real backend, not `mock`.** `--all-features` enables `mock`, a non-additive backend-replacing feature; both `test-windows` (task #858/F2) and `test-macos` (task #856/W14-2) used `--all-features`, so they exercised the mock recording stub instead of `VirtualAlloc`/`MEM_DECOMMIT` or `mmap`/`madvise`. Proven by an executed counterfactual: with `commit_len == size` deleted from `win_reserve_commit`'s guard, `cargo test --features lazy-commit --test lazy_commit` fails but `cargo test --all-features --test lazy_commit` still passes — that guard had zero non-vacuous CI coverage anywhere. Added a second step to both jobs with an explicit mock-excluding feature list (`lazy-commit huge-pages fault-injection bench-internals`); verified locally that `fault_injection.rs` goes 0 → 5 tests under it. R2 (fault-injection's Windows coverage) closed for free by the same change. **CR1 correction (same pass):** the initial landing of this task *deleted* the pre-existing `--all-features` step on both jobs instead of supplementing it, leaving a dangling comment describing a step that no longer existed and un-covering round-3's own F1 fix (the `page_size()` vs `PAGE` assertion) on the exact macOS/arm64/16 KiB-page runner F1 was written for. Restored both deleted steps in the round-4 closing pass; both jobs now run two `-p aligned-vmem` steps each, verified by parsing `ci.yml` with `yaml.safe_load` and enumerating the steps.
- **[test(vmem), P2] Task #868 (commit `f1f4cc4`) — R4: added `is_huge()` regression coverage.** `Reservation::is_huge()` had zero test assertions despite 3 rounds rewriting its contract (V3 introduced it, W2/W3 corrected it, F8/F9 corrected it again). Added `ordinary_reservation_never_reports_huge` (`tests/smoke.rs`) and a `#[cfg(not(target_os = "linux"))]` assertion in `tests/huge_pages.rs`. Counterfactual-verified in a scratch copy: reverting W2's Unix fix does not fail either assertion *on this Windows host* (W2 was Unix-only), but does fail on non-Linux Unix, which is exactly the macOS-row-shaped coverage the finding asked for.
- **[fix(vmem), P1, publish-gate] Task #869 (commit `36058de`) — R5: removed `Clone` from `ReservationParts` before the 0.2.0 publish.** `ReservationParts` derived `Clone`, letting the single-use release token it exists to make safer be duplicated in ordinary safe code and handed to `release_parts` twice (double `VirtualFree`/`munmap`) — reopening, one field over, the exact transposition footgun the type was introduced (task #845/V8) to close. Removed `Clone`; `as_tuple(self)` already gives a `Copy`-tuple escape hatch. Whole-repository grep confirms no internal `.clone()` call on a `ReservationParts` value anywhere. This was the one item round 4 called a hard publish gate (removing a derived `Clone` after 0.2.0 ships would be breaking); doing it now, before publish, is free — closing-review CR9 separately flags an unrelated, pre-existing (not this round's) publish-relevant `mock`-deferral text that also needs a maintainer decision.
- **[docs(vmem), P2] Task #870 (commit `3b63676`, closing-review CR5 residue fixed) — R3: fixed 3 inaccuracies task #859 introduced in the root crate's forwarding docs.** Task #859 (round-3 F3) itself introduced: (1) calling `WIN_ALLOCATION_GRANULARITY` (64 KiB) "the OS page size" (wrong by 16×; the real page size is 4 KiB, `dw_page_size`); (2) stating only half the dispatch condition (`align <= 64 KiB`, omitting `commit_len == size`) in two accessor docs; (3) "reset all four" naming five entries, one a derived sum. Fixed all three cited sites. **CR5 correction (closing pass):** the identical half-condition omission survived, unfixed, in a *third* accessor sixteen lines above the two R3 cited (`dbg_windows_reserve_commit_calls`, the derived-sum accessor and the actual oracle surface `examples/r32_13_windows_reserve_commit_decomposition_gate.rs` reads) — corrected in the same pass.
- **[docs(vmem), P2] Task #871 (commit `1519e0c`, closing-review CR6 residue fixed) — R6/R7: fixed the 5th over-reserve doc drift + stale huge-page justification + added a drift guard.** R6: the crate's top-of-file module rustdoc (the docs.rs landing page's first paragraph) still unconditionally described the over-reserve mechanism — the fifth drift of this sentence family (W5 fixed it 3×, F4 a 4th time). Reworded to match the already-correct wording elsewhere; added `scripts/vmem-doc-drift-guard.mjs`, wired into `scripts/check-all.mjs`, to catch a sixth recurrence. R7: `unix_reserve`'s huge-page doc justified its size/align restriction with a "head provably 0" argument about head/tail `munmap` calls task #842 deleted, and the "head provably 0" half was false in general for `align > LINUX_HUGE_PAGE_SIZE`. Fixed the cited paragraph. **CR6 correction (closing pass):** the identical stale head/tail-`munmap` description survived, unfixed, in a *different* paragraph of the same doc comment, ten lines above the one R7 cited — rewritten in terms of the surviving whole-mapping `munmap` in the same pass. **CR2 (guard weakness, NOT fixed in this pass, left open):** the round's own zero-trust review found and fixed a real substring bug in the guard (`"conditional"` matching inside `"unconditionally"`), but the round-4 closing review found the fix does not go far enough — the guard's qualifier is an OR (`align|conditional|Windows`), and every real historical drift sentence also contains "align" or "Windows", so the guard passes clean on the verbatim round-3 F4 sentence and the verbatim round-4 R6 sentence when reinjected as a counterfactual. The guard's header already documents a related, narrower limitation; CR2 recommends a per-sentence positional rewrite, not attempted here to avoid a rushed rewrite of the heuristic under this round's own time pressure.
- **[docs(vmem), P2] Task #872 (commit `3da36f8`) — R12: wrote the missing #858-864 CHANGELOG entry + filed the recurring "no CHANGELOG entry" pattern.** The entry immediately above this one (round-3 follow-up) had never been written — the third consecutive round with this exact gap (W16 → F11 → this). Wrote it, independently re-verifying all 7 cited SHAs. Filed the *pattern* (not just the instance) as a new `docs/CORRECTNESS_OPEN_ITEMS.md` `[A]` item, proposing (not self-adding) a standing CLAUDE.md rule. **CR4 correction (closing pass):** the new card itself mis-attributed the round-2 gap's closure to commit `7663811` (which actually closed the round-1 gap and is the campaign's own fabrication-incident commit) and mis-named the round-1 closing task; both citations corrected in this pass.
- **[docs(vmem), P3] Task #873 (commit `bb729bf`, closing-review CR3 regression fixed) — R9/R10/R11/R13: four small INFO-level doc/comment corrections.** R9: corrected the bench-internals design comment's "always compiled" storage claim (storage is also `bench-internals`-gated). R10: noted that F7's `debug_assert!` in `query_os_page_size()` never fires on the Windows reservation fast path it might be mistaken for covering. R11: two counter-fidelity doc corrections (Windows counters count successful calls only; the Unix hit-rate denominator includes OS-level failures, not just alignment misses). R13: replaced CLAUDE.md's stale `crates/vmem/src/lib.rs` single-file-crate example (the crate has been 4 files for a while) with `crates/numa/src/lib.rs`. **CR3 correction (closing pass):** in the course of R9's own fix, this task *reverted* task #859's round-3 F3 correction of `crates/vmem/Cargo.toml`'s "`#[doc(hidden)]` accessors" clause back to "accessors" from "statics" — justified by sefer-alloc's own convention (correct about sefer-alloc), but the sentence's subject is `aligned-vmem`, which does NOT mirror that attribute placement (its statics, not its accessors, are `#[doc(hidden)]`), and it desynchronized the round's own new CHANGELOG entry (which still said "statics") from the code. Rewritten in the closing pass so the sentence names both crates' actual attribute placement unambiguously, and cannot silently invert again.
- **[bench, P3] Task #874 (commit `a9eca93`) — R8: wrote a design note for the VirtualAlloc2 VA-space optimization opportunity.** Design-only (no `crates/vmem/src/` change, confirmed by `git show --stat`): the Windows `align > 64 KiB` path costs 2 syscalls and `size + align` VA; `VirtualAlloc2` + `MEM_ADDRESS_REQUIREMENTS` could do 1 syscall with no over-reserve, gated on an unresolved Windows-version-floor policy question (VirtualAlloc2 is kernelbase.dll, not kernel32.dll). All cited R32-13 figures independently re-verified against that report by the closing review and matched exactly. **CR7 (not fixed in this pass, cosmetic):** the note's own TL;DR bullet describes the current path as a 3-call "dance" (adds a phantom `VirtualFree`) in the same sentence that correctly says "2" — the rest of the note, and the code, are right; only that one bullet needs a wording fix.

All 13 findings from the round-4 review are now closed, and 6 of the round-4 closing review's 10 new findings (CR1, CR3, CR4, CR5, CR6, CR8) were fixed in this same pass; CR2, CR7, and CR9 are explicitly left open above with reasons. `aligned-vmem`'s own crates.io publish decision (task #658) has its one round-4-identified hard gate (R5) closed; CR9's pre-existing `mock`-deferral text still wants a maintainer decision before publish. **None of rounds 1-4's work has ever run in CI as of this entry** — confirm the landing SHA goes green on `origin/main` before treating any of it as more than a local, single-host claim.

#### `aligned-vmem` — round-5 follow-up (2026-08-13, tasks #875-879)

A fresh review of the post-round-4 tree (`docs/reviews/2026-08-13-aligned-vmem-round5-review.md`, findings Q1-Q9) found the campaign's own predicted highest-yield technique — execute an artifact and see what it actually catches, rather than read it — still had real yield: Q1's regression test for bug #848 was vacuous by construction (the assertion whose message names the invariant, `assert_eq!(r.len(), size, "the reservation must cover the full requested span")`, can never fail because `len` is copied straight from the caller's request; an executed counterfactual showed the suite fails 17 lines lower, at a different assertion). All 5 tasks delegated to `/crush` with full personal zero-trust re-verification (including two real defects caught and personally fixed on top of the delegates' own commits, before merge — see #878/#879 below), followed by a dedicated round-5 CLOSING review (`docs/reviews/2026-08-13-aligned-vmem-round5-closing-review.md`, findings QC1-QC9) that caught the same recurring shape one level deeper: not in the delegates' mechanism, but in the *finding's own prescribed fix text*, quoted verbatim into source without anyone re-testing the sentence. QC1 and QC2 (both MEDIUM) and QC3-QC8 (LOW/INFO) were fixed in this same pass. **This is a sub-crate follow-up — `aligned-vmem` is a workspace member library not consumed by `sefer-alloc`'s own `production` feature bundle, so none of these changes affect the allocator's shipping runtime.**

- **[test(vmem), P1] Task #875 (commit `8f6284b`) — Q1: fixed the vacuous #848 regression-test assertion.** `tests/lazy_commit.rs`'s `assert_eq!(r.len(), size, ...)` is an identity check on the caller's own input (`finish_reservation` hardcodes `len: size` on every path) and cannot discriminate the bug it exists to catch. Added `assert!(r.reservation_len() >= size, ...)` as the real two-sided oracle and corrected the original assertion's message to stop claiming to check the reservation. Personally re-verified the counterfactual myself: with `commit_len == size` deleted from `win_reserve_commit`'s guard, the suite now fails **at the new assertion** with "got 4096" — the assertion whose message names the invariant now also checks it.
- **[docs(vmem), P2] Task #876 (commit `48b7219`) — Q2: fixed the half-stated Windows dispatch condition in `lib.rs` + `Cargo.toml`.** The real condition is `align <= 64 KiB` **and** `commit_len == size`; two sites inside `aligned-vmem` itself (the `bench-internals` design comment, and the feature doc that ships inside the published `.crate` tarball) stated only the first half — the same defect class R3/CR5 already fixed in the root crate's forwarders. Both corrected to name the full condition.
- **[ci(vmem), P2] Task #877 (commit `c2922f3`) — Q3: added the missing clippy row for the real (non-mock) backend.** Round-4's R1 fixed `cargo test` coverage but left both `cargo clippy` rows on the `--all-features`/default split, which inherits the identical `mock`-erases-the-real-branch gap for `try_commit_range`'s real-backend arm. Added a third row with the everything-except-`mock` feature list, verified green as a from-scratch (non-cached) recompile by the closing review.
- **[fix(vmem), P1] Task #878 (commits `917cde9` + `aafb35d`) — Q8+Q4: rewrote the doc-drift guard to a per-sentence predicate (closes round-4's CR2), fixed a sixth over-reserve doc drift.** CR2 (left open in round 4) found the guard's OR-qualifier (`align|conditional|Windows`) let every real historical drift sentence pass, because those words are what the drift sentences are made of. Rewrote the predicate as `TRIGGER && (HARD_FAIL || !SCOPE)`, evaluated per sentence: `unconditional` now convicts outright, and a real scope word (`if`/`when`/`<=`/`fast-path`/etc) is required to pass. Verified against CR2's own two decisive counterfactuals (the verbatim round-3 F4 and round-4 R6 drift sentences) — both now fail the guard. Also fixed Q4 (a sixth instance of the same sentence family, `from_raw_parts`'s rustdoc). **Personal fix (before merge):** the delegate's claimed widening of the guard's file scope to `Cargo.toml`/`README.md` was non-functional — it read both files' content but the doc-block accumulation was gated on `///`/`//!` line prefixes, which TOML/Markdown never contain, so the check never actually ran on either file (caught by injecting a drift sentence into `Cargo.toml`'s `description` and watching it pass clean). Fixed with a separate per-line scan path for non-`.rs` files; the closing review independently confirmed this fix is load-bearing on a real historical drift, not just a synthetic probe.
- **[docs(vmem), P2] Task #879 (commits `c223ae4` + `55fb6f0`) — Q5+Q6+Q7: mis-citation fix, publish-task number correction, `docs/CORRECTNESS_OPEN_ITEMS.md` archival.** Q5: fixed a guard-header mis-citation CR8 had already fixed in `check-all.mjs` but left in the guard's own file (resolved a genuine merge conflict against task #878's guard rewrite by combining both citations). Q6: corrected `#659` → `#658` (aligned-vmem's actual publish task) at two sites, per CR9. Q7: moved a closed `docs/CORRECTNESS_OPEN_ITEMS.md` item's full closure narrative to "Recently resolved", per CLAUDE.md's R34-24 structural rule. **Personal fix (before merge):** the delegate's Q6 restatement in `Cargo.toml` was a grammatically truncated sentence with a stray double-hash line (`## and converting now would be a large mechanical change`) — rewritten to a complete sentence matching the parallel fix already applied to `src/mock.rs`.

**Closing-review findings (QC1-QC9), fixed in this same pass except where noted:**

- **QC1 (MEDIUM, publish-relevant, fixed).** Q6's "honest premise" replacement was itself false, in the decision-inverting direction: `mock` (and `Call`, and `src/mock.rs`) was introduced in the same commit (`4ec1516`) that bumped 0.1.0 → 0.2.0 — it was never part of a published version. "Removing `mock` is already a breaking change" is wrong; the free window to convert it to a `--cfg` flag is still open and closes only at `cargo publish` (task #658). Corrected all three sites (`Cargo.toml`, `src/mock.rs` ×2) to state the real boundary. CR9's underlying `--cfg`-vs-Cargo-feature design decision remains open for a maintainer — this only fixes the premise it will be read against.
- **QC2 (MEDIUM, fixed).** The personal fix to task #878 (making `Cargo.toml`/`README.md` scanning functional) was real and load-bearing, but the guard's `SCOPE` regex accepted a bare `->`/`<T>` from a Rust function signature as a "qualifier" — every row of `README.md`'s API table contains an arrow, so any over-reserve drift written into that table (including the crate's own most-drifted sentence, five recurrences across five rounds) was invisible to the guard by construction. Fixed by stripping inline code spans before the `SCOPE` check (not before `TRIGGER`, since real qualifiers like `` `align <= 64 KiB` `` legitimately live inside backticks in `.rs` rustdoc). Re-verified: the exact README counterfactual the closing review used is now caught, and the clean tree still passes.
- **QC3 (LOW, fixed).** Task #878's own Q4 fix wrote a fresh instance of the exact half-stated dispatch condition Q2 was fixing 430 lines earlier in the same file, in the same round (missing the `commit_len != size` disjunct) — plus a second site (`lib.rs`'s two-call-path inline comment) that Q2's own prescribed `grep -rn '64 KiB'` sweep should have found and didn't. Both corrected.
- **QC4/QC5 (LOW, fixed).** `scripts/check-all.mjs` still described the guard's deleted OR-qualifier predicate (the exact predicate CR2 condemned) as what the pre-push gate checks; the guard's own header framed "contains `unconditional`" as a way to *satisfy* the requirement when the code treats it as an outright conviction. Both rewritten to match the actual per-sentence predicate.
- **QC6/QC7 (INFO, fixed).** Two stale line-range citations into `crates/vmem/Cargo.toml` (in `ci.yml` and `docs/CORRECTNESS_OPEN_ITEMS.md`) that Q6's own edit shifted by one line, within the same round that wrote them — replaced with citations by feature name instead of line range. The `aligned-vmem-gates` job's header comment still said "two clippy rows are required" with three now present — added the third's clause.
- **QC8 (INFO, fixed).** Q1's own review text flagged, as a coverage gap in its own right, that no test asserts `reservation_len()` on the Windows single-call fast path — the one path where it deliberately does *not* report the true OS reservation size. Added one assertion in `tests/smoke.rs` pinning it.
- **QC9 (INFO): this entry.** The same "no CHANGELOG entry yet" gap CR10 caught for round 4 recurred for round 5's own tasks — caught by the closing review and closed in this same pass rather than aging into round 6.

All of Q1-Q8 are closed and independently re-verified by the closing review (Q9 required no action). `aligned-vmem`'s crates.io publish decision (task #658) gains one new pre-publish-relevant item from this round: QC1's corrected premise means the `--cfg`-conversion window CR9 defers is confirmed still open today and closes at publish, not before. **None of rounds 1-5's work has ever run in CI as of this entry** — `origin/main` is 29 commits behind local `main`; confirm the landing SHA goes green before treating any of it as more than a local, single-host claim.

#### `aligned-vmem` — macOS decommit CI-discovery fix (2026-08-13, commit `9c777bc`)

The push that finally sent rounds 1-5's accumulated commits to `origin/main` turned `main` red: `test macos (production)` failed on `decommit_recommit_roundtrip` (`crates/vmem/tests/smoke.rs`), the first time this crate's real (non-mock, non-miri) suite had ever executed against real macOS hardware (task #867/R1 added the CI row two rounds earlier; the push itself had been deferred until now). **This is the campaign's first CI-discovered defect** — every finding in rounds 1-5 (V/W/P/F/R/Q, all their closing-review CR/QC follow-ups) was caught by a review reading the code, not by CI turning red.

The failure: a byte written before `decommit()`+`recommit()` was still readable afterward — `left: 119, right: 0` — where the identical assertion passed on both Linux and Windows in the same CI run. Root cause: `recommit_pages_impl`'s Unix implementation is an unconditional no-op on every Unix target, justified by a comment claiming re-access after `MADV_DONTNEED` is implicitly zero-filled — true on Linux, **false on macOS**, where `MADV_DONTNEED` is advisory-only for anonymous memory and the kernel is free to leave the old contents in place.

Fixed by scoping `decommit_recommit_roundtrip`'s zero-fill assertion off macOS (`#[cfg(not(any(miri, feature = "mock", target_os = "macos")))]`) and adding an explicit macOS caveat to `decommit()`'s rustdoc and `recommit_pages_impl`'s code comment, matching the existing huge-page-no-op precedent already documented in the same doc comment. Filed as `docs/CORRECTNESS_OPEN_ITEMS.md` item 48 in the same commit, per CLAUDE.md's phased-delivery rule. **This is a documentation/test-scoping fix, not a behavior change** — `decommit()`'s actual macOS behavior is unchanged; the underlying gap (silently missing RSS return and zero-fill on Darwin) remains open, tracked as item 48, with `mmap(MAP_FIXED | MAP_ANONYMOUS)` re-mapping recorded as the candidate real fix for a future round.

A round-6 review of the post-`9c777bc` tree (`docs/reviews/2026-08-13-aligned-vmem-round6-review.md`, findings S1-S12) found this mitigation left 4 further public statements of the now-false guarantee standing (S1), found the CI evidence does not actually distinguish "MADV_DONTNEED is advisory" from "the madvise call failed silently" (S2), and found the "previously-undiscovered" framing above is true only at this crate's own scope — the same fact has been documented repo-wide, including in `.github/workflows/ci.yml` itself, since round 9 of the wider campaign (S3). Those findings and the rest of S1-S12 were fixed by round 6's own 7 follow-up tasks (#880-886, see the entry immediately below) — landed and merged, not merely tracked.

#### `aligned-vmem` — round-6 follow-up (2026-08-13, tasks #880-886)

A fresh review of the post-`9c777bc` tree (`docs/reviews/2026-08-13-aligned-vmem-round6-review.md`, findings S1-S12) found the mitigation for the campaign's first CI-discovered defect (see the entry above) left real residue — 4 more public statements of the false zero-fill guarantee (S1), an unestablished root-cause attribution for item 48 (S2), a lost effect-oracle on macOS (S4), and the "previously-undiscovered" framing being true only at this crate's own scope, not the repository's (S3). This round was delegated differently from rounds 1-5: 7 parallel `Agent` instances (`subagent_type: "sh"`, Sonnet, high effort) each in its own isolated git worktree, instead of `/crush` — the first time this campaign used that delegation path. Each was personally zero-trust reviewed and merged one at a time, including 3 personal fix commits on top of the delegates' own work (a SERIAL-mutex race fix on the new macOS test oracle, a README wording alignment, and a "discovered" → "confirmed" phrasing fix repeated in two files) and two hand-resolved merge conflicts on `docs/CORRECTNESS_OPEN_ITEMS.md` item 48 (two/three parallel worktrees had all added content to the same item). A dedicated round-6 CLOSING review (`docs/reviews/2026-08-13-aligned-vmem-round6-closing-review.md`, findings SC1-SC10) then found the campaign's pattern held for a sixth time: two MEDIUM findings were direct products of worktree isolation (SC2: two different agents wrote 6 sentences with inconsistent Darwin-family scoping, textually non-conflicting so no merge conflict flagged it; SC3: this very CHANGELOG gap, since the agent that wrote the *previous* entry's closing sentence could not see the 7th task's existence in its own worktree) and one was the round-3 lesson recurring (SC4: a crate-scoped review changing a cross-crate function's *contract*, not its signature, which nothing in the compiler or test suite could flag). All ten SC findings were fixed in this same closing pass.

- **[docs(vmem), P1] Task #880 (commit `991bf4d`) — S1: added a macOS cross-reference to the 4 sites still stating the false zero-fill/no-op guarantee.** `recommit()`'s entire rustdoc, `decommit()`'s opening sentence, the crate-root module doc, and `decommit_lazy()`'s rustdoc all still stated or implied the pre-`9c777bc` guarantee with no pointer to the new caveat. Added one cross-reference sentence to each.
- **[docs(vmem), P2] Task #881 (commit `e7ad1b7`, zero-trust wording fix folded in) — S5: added a README "Platform caveats" section.** The crates.io landing page's only statement of `decommit`/`recommit` semantics was one table row; the rustdoc spends paragraphs on 3 platform divergences (Windows crash-not-refault, huge-page no-op, macOS zero-fill gap) the README never mentioned. Added a scannable `## Platform caveats` section for all three, each pointing at the relevant function's own rustdoc. **Personal fix (before merge):** the delegate's macOS-gap bullet said "Discovered 2026-08-13"; reworded to match task #883/S3's more accurate framing.
- **[feat(vmem), P1] Task #882 (commit `e839b1a`, SERIAL-mutex race fix folded in) — S2+S4: bench-internals-gated madvise-success oracle for macOS decommit.** Item 48's stated root cause ("`MADV_DONTNEED` is advisory-only on Darwin") was asserted from one failing byte, equally consistent with the `madvise` syscall itself having failed — `libc_madvise` discards the return value by design, so nothing could tell the two apart. Added two new `bench-internals`-gated counters (`UNIX_MADVISE_ATTEMPTS`/`UNIX_MADVISE_SUCCESSES`) and a macOS-gated test asserting the syscall itself returns success for both the eager and lazy decommit paths — settling the question on the next real macOS CI run, restoring the effect-observing coverage `9c777bc` removed. **Personal fix (before merge):** the new test resets and reads process-global counters that 3 OTHER tests in the same file also touch, and libtest runs a binary's tests on parallel threads by default — added a `SERIAL: Mutex<()>` (mirroring `tests/fault_injection.rs`'s own existing precedent for the identical hazard) to all 4 decommit-calling tests.
- **[docs, P2] Task #883 (commit `989a1e9`) — S3+S12: corrected item 48's "previously-undiscovered" framing, fixed a mis-citation in two perf design docs, folded a perf idea into item 46.** The macOS `MADV_DONTNEED` hazard was not newly discovered — it was already documented in `.github/workflows/ci.yml`, a production code comment, and two `virgin-zero-skip` design docs since Round 9; only this extracted crate's own docs/tests had never reflected it. Corrected item 48's framing and annotated `R9_5`/`R11_8`'s citation of a `lib.rs` note that didn't exist when they were written. S12 (a Linux/macOS `mmap` hint-retry perf idea, unmeasured) recorded in `docs/perf/OPEN_ITEMS.md` item 46's next-trigger text.
- **[test(vmem), P2] Task #884 (commit `63427af`) — S6: added an Apple-Silicon 16 KiB page-size assertion, corrected item 43's verification claim.** Item 43 claimed the macOS `_SC_PAGESIZE` constant was "verified" on the strength of a Windows-host cross-compile check, which proves compilation, not correctness — and the crate's own generic page-size test would pass even on a silently-wrong constant, because `page_size()`'s fallback value (4 KiB) also satisfies it. Added a hard assertion (`page_size() == 16 * 1024`) gated to real Apple Silicon, which this crate's CI already runs on.
- **[docs(vmem), P2] Task #885 (commit `79821d6`, "discovered" phrasing fix folded in) — S7+S8+S9: widened the macOS caveat to the Darwin family, fixed a `reservation_len()` uniqueness overclaim, recorded a `MADV_FREE_REUSE` alternative.** The crate already treats macOS/iOS/tvOS/watchOS as one XNU family for other constants; widened the decommit caveat's `#[cfg]` and doc scope to match. Corrected `reservation_len()`'s rustdoc, which claimed the Windows fast path was the ONE under-reporting case (a 16 KiB-page macOS host has a second, identical-shape case). Recorded (not implemented) a cheaper partial alternative fix for item 48's Next-trigger section. **Personal fix (before merge):** two more "discovered 2026-08-13" instances (a doc heading, a test comment) that task #883/S3 was scoped not to touch, reworded to match S3's framing.
- **[fix(vmem-tooling), docs, P2] Task #886 (commit `e7303dd`) — S10+S11: fixed the doc-drift guard's self-contradicting header/regex, wrote the `9c777bc` CHANGELOG entry (the entry immediately above).** The guard's `SCOPE`-check stripped every backtick-wrapped code span to stop a Rust signature's `->` arrow from falsely "rescuing" a drift sentence — but the crate's own real qualifiers (e.g. `` `align <= 64 KiB` ``) legitimately live inside backticks too, so the strip convicted correctly-qualified sentences. Narrowed the strip to just the arrow/generic tokens causing the false rescue, independently re-verified against both the original arrow-rescue case and the two CR2-era historical drift sentences (all still correctly handled).

**Round-6 closing review findings (SC1-SC10), all fixed in this same pass:**

- **SC1/SC4/SC5/SC6 (MEDIUM/LOW-MEDIUM/LOW/LOW, fixed).** S4's second limb (`decommit_lazy_roundtrip` has zero effect-observing coverage on any platform) was left open by task #882 and its new oracle's own rustdoc asserted the opposite; corrected the rustdoc and recorded the remainder in item 48. `reset_bench_internals_counters()` grew from four counters to six but three enumerations of it (including one in the ROOT crate's own `#[doc(hidden)]` forwarder) stayed stale — the root-crate blind spot that produced task #864 in round 3, recurring; corrected all three. `R9_5`/`R11_8`'s citation-accuracy annotation over-claimed "now accurate" when S3 had named two specific respects (BSD, `decommit_lazy`) it still wasn't; narrowed the claim. `decommit_lazy()`'s new macOS cross-reference flattened the exact cost-ordering inversion item 48's own S9 note records (lazy is the one that actually drops footprint on macOS/iOS, not eager); rewrote to state both axes precisely.
- **SC2 (MEDIUM, fixed).** Two parallel worktrees (#880, #885) each wrote "macOS"-scoped cross-references while the other was independently widening a different paragraph to "the Darwin family" — 6 sites (including the brand-new README section) still said "macOS" after both merged. Unified all sites to "Darwin family (macOS/iOS/tvOS/watchOS)" and added an explicit note that the *lazy* path's footprint-drop benefit is macOS/iOS-specific (tvOS/watchOS fall back to the same `MADV_DONTNEED` no-op as the eager path there).
- **SC3 (MEDIUM, fixed — this entry).** Round 6 had no CHANGELOG entry for its own 17 commits; the one sentence that mentioned it (in the entry above) was off by one task (`#880-885` omitting `#886`, the very task that wrote it) and described already-merged work as a pending follow-up. This entry closes it.
- **SC7 (LOW, fixed).** Item 48's Status card and internal bullet order were never resynchronised after two hand-resolved merge conflicts added a second "Root cause" bullet and nested the S9 note inside "Next trigger" instead of standing on its own — reordered and updated the Status line to reflect everything round 6 actually landed.
- **SC8/SC9/SC10 (INFO, fixed).** A new counter's rustdoc said a private helper was "named in code font rather than linked" while linking it anyway; corrected to bare backticks. The new test's counter reset clobbers two unrelated counters no other test asserts on today; added a one-line note on the reset call. The oracle's `attempts == 2` exact-count assertion will misfire if a future round implements item 48's recorded `MADV_FREE_REUSE` alternative (a second counted call per cycle); added a forward-pointing comment.

All of S1-S12 are closed (S2 and S6 pending the next real macOS CI run for final empirical confirmation — both are correctly gated and will execute automatically). `aligned-vmem`'s crates.io publish decision (task #658) gains no new blocker from this round; the README `## Platform caveats` section (S5, now Darwin-scoped per SC2) is publish-relevant and now accurate. **None of round 6's work has been pushed as of this entry** — confirm the landing SHA goes green on `origin/main`, including the two headline artifacts (`macos_decommit_madvise_syscall_actually_succeeds`, `apple_silicon_page_size_is_16_kib`) whose entire evidentiary purpose is to run in real CI, before treating S2/S6 as settled.

**Follow-up (2026-08-13, task #888, finding T1 of `docs/reviews/2026-08-13-aligned-vmem-round7-review.md`): the closing instruction immediately above is now satisfied.** Commit `1dbd6b4` was pushed and CI run `31692217669` (job `94421845398`, `test macos (production)`, image `macos-26-arm64`) ran green, executing both headline artifacts — `apple_silicon_page_size_is_16_kib` and `macos_decommit_madvise_syscall_actually_succeeds`, both `ok`. S6 is now settled (moved to `docs/CORRECTNESS_OPEN_ITEMS.md`'s "Recently resolved" as item 43's macOS half). S2 is partially settled: the run rules out H2 (the `madvise` syscall itself failing — `unix_madvise_attempts() == 2 && unix_madvise_successes() == 2`), leaving H1 (Darwin's `MADV_DONTNEED` is advisory-only) as the only remaining explanation, combined with run `31676133649`'s stale-byte evidence from a different commit — item 48's Root-cause bullet records the precise two-run wording rather than an overstated "H1 confirmed by CI."

#### `aligned-vmem` — round-7 follow-up (2026-08-13, tasks #888-894)

A fresh review of the post-`1dbd6b4` tree (`docs/reviews/2026-08-13-aligned-vmem-round7-review.md`, findings T1-T10) found round 6's own SC1-SC10 fixes held completely — the first round in this campaign where the closing pass's own diff did not generate the next round's headline — but that the world had changed underneath the repository instead: `1dbd6b4` was pushed (per the entry immediately above) and CI answered two questions the indexes still recorded as open (T1); a scope-word gap survived SC2's Darwin-family sweep one axis over, in the *advice-selection* sentences rather than the *decommit-gap* sentences it actually fixed (T4); a near-vacuous test's doc comment misattributed itself (T2); a new test was missing a `not(miri)` gate its four siblings all carry (T3); seven publish-facing citations pointed at a file not in the published package (T5); and four INFO-level hygiene items (T6-T10) rounded it out. This round was again delegated to 7 parallel `Agent` instances (`subagent_type: "sh"`) in isolated worktrees (`vmem-r7-a` … `vmem-r7-g`, branched from `1dbd6b4`), each personally zero-trust reviewed and merged one at a time — all seven merges were clean, no conflict markers, no mis-citation landed on the wrong content (a better record than round 6's parallel delegation produced).

- **[docs, P1] Task #888 (merge `0cdb596`) — T1: recorded round-6's real macOS CI evidence in items 43 and 48.** Moved item 43's macOS half to "Recently resolved" per its own pre-written Next-trigger instruction; updated item 48's Root-cause bullet to record H2 as excluded and H1 as the remaining explanation, using the exact two-run wording the review insisted on (NOT a flat "H1 confirmed by CI" — the stale-byte and madvise-success evidence come from two different CI runs, never observed together in one process).
- **[docs, P2] Task #889 (merge `c1867d8`) — T5: replaced 7 publish-facing `docs/CORRECTNESS_OPEN_ITEMS.md` citations with resolvable URLs.** `cargo package -p aligned-vmem --list` has no `docs/` directory (it lives at the workspace root, outside the package), so bare-path citations in README.md/lib.rs/Cargo.toml resolved to nothing for a crates.io/docs.rs reader. Converted to absolute GitHub URLs.
- **[test(vmem), P3] Task #890 (merge `ceaa3fe`) — T3: excluded `apple_silicon_page_size_is_16_kib` under miri.** The crate's own `#[cfg(miri)]` arm of `query_os_page_size()` unconditionally returns 4 KiB, so the test's exact-16-KiB assertion failed by construction under miri — not a real bug, a missing `not(miri)` gate matching the pattern all four sibling real-OS-property assertions already use.
- **[docs(vmem), P2] Task #891 (merge `ac08342`) — T4: fixed macOS-vs-macOS/iOS scope in `decommit_lazy`'s advice documentation.** `MADV_FREE_REUSABLE` applies to macOS AND iOS in the code, but 5 doc sites said "macOS" alone — including one that directly contradicted a paragraph 9 lines below it in the same rustdoc. Fixed all 5 sites named by the review.
- **[test(vmem), P3] Task #892 (merge `e496071`) — T2: corrected a near-vacuous test's doc comment.** `ordinary_reservation_never_reports_huge`'s doc comment claimed it regression-guards W2, but `reserve_aligned`'s ordinary path hard-codes `granted_huge: false` as a literal — the assertion cannot fail against the bug it claimed to guard. Rewrote the doc comment to state what the test actually pins and named `huge_pages.rs:61-62` as the real W2 regression test.
- **[docs(vmem), P3] Task #893 (merge `b569cfe`) — T6: documented `MAP_ANON`/`MAP_HUGETLB`'s architecture dependency (REASONED-FROM-SPEC).** Both constants are gated on `target_os = "linux"` alone but their VALUES are architecture-dependent (wrong on MIPS); added a prose note matching the `_SC_PAGESIZE` table's existing precedent, no `#[cfg]` change (no currently-supported target is affected).
- **[fix(vmem), docs, P2] Task #894 (merge `1e532a7`) — T7+T8+T9+T10: INFO-level hygiene bundle.** Fixed the one concrete bug within T7 (rustfmt had glued an `mmap` call's `// SAFETY:` comment onto the trailing comment of the preceding unrelated line); the other 9 implicit-`unsafe-fn`-body sites filed as item 49, not silently dropped. Added the missing `// SAFETY:` comment to `win_reserve_commit`'s single-call retry (T8). Converted the mock recording path's 6 remaining exposed-address `ptr as usize` casts to `.addr()`, plus both showcase snippets (T9). Recorded S12's plumbing gap in `docs/perf/OPEN_ITEMS.md` item 46 (T10, documentation-only, S12 itself not implemented).

**Round-7 closing review findings (TC1-TC9, `docs/reviews/2026-08-13-aligned-vmem-round7-closing-review.md`), all fixed in this same pass:**

- **TC1/TC2 (MEDIUM/MEDIUM, fixed — this entry).** Round 7's decomposition assigned every finding an owner and assigned the round's own bookkeeping to nobody: no CHANGELOG section (this entry closes it; item 1's counter bumped to a 7th recurrence below) and the round-7 review doc itself was never committed, while 4 durable records already cited it by path (now fixed by committing both round-7 review docs in this same pass).
- **TC3 (LOW-MEDIUM, fixed).** Task D's new tvOS/watchOS rationale in `lib.rs` ("`MADV_FREE_REUSABLE` is XNU-wide, so all four Darwin targets could in principle use it") asserted the exact opposite of item 48's untouched S9 bullet ("would not help tvOS/watchOS at all") — a worktree-isolation collision one file and ~980 lines apart, no git conflict because the edits were prose, not overlapping text. Reworded both sites to agree: REASONED-FROM-SPEC, unverified on tvOS/watchOS hardware/target, a plausible widening candidate rather than a settled fact either way.
- **TC4 (LOW, publish-relevant, fixed).** T5's own "Where" clause named 9 sites; task B fixed 8. Swept the remaining `Cargo.toml:103` citation, plus one more (`lib.rs:262`, a `#[doc(hidden)]` doc comment that still ships in source) the review found outside T5's original list. Also caught and fixed a fresh instance of the same bare-citation pattern this pass's own TC3 fix had just introduced at `lib.rs:1131`.
- **TC5 (LOW, fixed).** Task C's `not(miri)` rationale cited "`decommit_lazy_roundtrip`'s sibling in `lazy_commit.rs`" as a precedent; no such test exists (`decommit_lazy_roundtrip` lives in `smoke.rs`, not `lazy_commit.rs`, and `lazy_commit.rs` never calls `decommit_lazy` at all). Renamed to the real precedent: `smoke.rs`'s `recommit_is_fallible_and_reports_success_on_the_happy_path`, mirrored by `lazy_commit.rs`'s `sequential_commit_range_grows_incrementally`.
- **TC6 (LOW, fixed).** The madvise oracle's own rustdoc still described the confirming CI run as future work ("on the next real macOS CI run") after task A's index-card updates made the same claim past tense. Updated the rustdoc to cite the actual run (`31692217669`, job `94421845398`) with the same two-run H1 caveat item 48 carries.
- **TC7 (LOW, fixed).** Item 49's current-state card was missing the `Current-number-or-verdict` field CLAUDE.md's R34-24 rule requires. Added it without hardcoding fresh line numbers (the file keeps changing) — states the 9-of-10-remaining count and the exact `RUSTFLAGS` invocation to re-derive current locations.
- **TC8/TC9 (INFO, fixed).** The README provenance caveat task G added described the mock backend as still exposing addresses via `as usize` — the very thing that same task's own commit had just eliminated by switching to `.addr()`. Reworded to state what's actually true. T6's architecture enumeration mislabeled an incomplete list "the tier-1/tier-2 targets" (s390x, loongarch64 are both tier-2 and both absent); reworded to state the rule (every Linux arch using `asm-generic/mman-common.h`) rather than an incomplete roster.

Nothing in round 7 or its closing fix pass changed runtime behavior — every hunk across both passes is a comment, doc comment, `#[cfg]` predicate on a test, or a `ptr as usize` → `ptr.addr()` swap (no observable difference). Publish-readiness (task #658) is not newly blocked.

#### `aligned-vmem` — round-8 follow-up (2026-08-13, tasks #897-903)

A fresh review of the post-`8380607` tree (`docs/reviews/2026-08-13-aligned-vmem-round8-review.md`, findings U1-U11) found round 7's own TC1-TC9 fixes held cleanly, and produced the campaign's first genuine soundness-adjacent code finding since round 3: **U1 (MEDIUM)** — `try_reserve_aligned_exact`'s `align > page_size() && !region_addr.is_multiple_of(align)` alignment check silently skipped validation whenever `align <= page_size()`, reasoning that `mmap` always returns page-aligned addresses in that range. That reasoning is correct only if `page_size()` is <= the REAL OS page size — three prior rounds (3, 4, 7) each examined these exact lines and cleared them by checking only that safe direction; none checked the unsafe direction, where a wrong `_SC_PAGESIZE` constant on one of item 43's still-open BSD targets returning a power-of-two ABOVE the real page size would silently violate `Reservation::as_ptr()`'s documented alignment guarantee with no error. Fixed by deleting the conjunct (the review's own measurement showed it saved zero syscalls). The other ten findings were smaller: a documented Windows crash footgun in `decommit_lazy` (U5), unattributed WSL2-only benchmark numbers on two publish-facing surfaces (U6), a citation fix that itself needed fixing (U2 — round 7's own TC5 correction had named the wrong test as the `not(miri)` precedent), two stale-citation corrections (U3, U8), new test coverage for an untested contract-violation half (U7), and four INFO items (U4, U9, U10, U11). This round was again delegated to 7 parallel `Agent` instances (`subagent_type: "sh"`) in isolated worktrees, each personally zero-trust reviewed and merged — all seven merges were clean, and this round produced the best mis-citation record of the campaign: all eleven fixes landed on the right content, none introduced a new wrong claim.

- **[fix(perf), P0] Task #897 (merge `491afe9`) — U1: removed the unsound `align > page_size()` skip in `try_reserve_aligned_exact`.** The check now runs unconditionally; the review's own closing pass independently re-derived the fix's correctness from first principles (traced every caller to confirm `align` reaching this function is always a validated power-of-two `>= PAGE`, so the newly-unconditional check has no new failure mode on any correct platform) rather than taking the task's argument on trust. Appended the second consequence (misalignment, not just decommit-rounding) to item 43's `Current-number-or-verdict` bullet.
- **[docs(vmem), P2] Task #898 (merge `a469643`) — U5: documented `decommit_lazy`'s Windows crash footgun.** Its rustdoc taught the Linux "a write afterwards is fine" mental model and never mentioned `recommit`, while on Windows the same call is the eager `MEM_DECOMMIT` path where a write before `recommit` is a hard `STATUS_ACCESS_VIOLATION` — the same crash class item 6 already records for `decommit` itself. Added the clause to both `lib.rs` and the README table row.
- **[docs(vmem), P3] Task #899 (merge `2195ad2`) — U6: attributed WSL2-only hit-rate numbers on two publish-facing surfaces.** The measured 34.4%/46.7%/56.7% hit-rate figures ship unqualified in `README.md` and `lib.rs`'s rustdoc, but their source card (`docs/perf/OPEN_ITEMS.md` item 46) explicitly labels them WSL2-only and not yet confirmed to transfer to bare-metal Linux or other Unix platforms. Added the qualifying clause to both surfaces, numbers re-verified against the source card rather than the review's restatement of them.
- **[test(vmem), P3] Task #900 (merge `ccc017e`) — U2: fixed a mis-citation that replaced a mis-citation.** Round 7's TC5 fix renamed a wrong `not(miri)` precedent citation to `recommit_is_fallible_and_reports_success_on_the_happy_path` — which itself has no `#[cfg]` gate and no zero-fill read, an error inherited from an even older comment (task #716) in a second file. Renamed to the real precedent, `decommit_recommit_roundtrip`, in both `smoke.rs` and `lazy_commit.rs`, preserving the full correction trail in both comments.
- **[docs, P3] Task #901 (merge `f654bda`) — U3+U8: replaced a stale line-number citation with symbol names, fixed item 1's stale headline count.** Item 48's S9 "keep in sync" bullet cited exact line numbers that the same commit that wrote them had already invalidated; replaced with symbol names (`decommit_lazy`'s rustdoc, `madv_free_advice`'s doc comment), immune to future line drift. Item 1's headline still said "recurred THREE times" while its own Current-number bullet already recorded a 7th instance — updated to "seven" and restructured so only one line needs to move per future round.
- **[test(vmem), P2] Task #902 (merge `f90a8a4`) — U7: added test coverage for decommit's untested silent-skip contract-violation half.** `decommit`/`decommit_lazy`'s contract is deliberately asymmetric with `recommit` — a violation is silently skipped, not rejected — and nothing tested that half at all; a plausible-looking "unify the validation base" refactor would have shipped silently broken on any 16 KiB-page host with the whole suite green. Added two tests (mock-layer and real-syscall-layer), with a personally-performed counterfactual verification (temporarily broke the guard, confirmed both new tests failed for the right reason, reverted).
- **[fix(vmem), docs, P2] Task #903 (merge `70df07e`) — U4+U9+U10+U11: INFO/LOW hygiene bundle.** Fixed a `// SAFETY:` proof copied verbatim onto a call it didn't accurately describe (U4). Narrowed an overclaimed provenance guarantee back to its correct scope and fixed the one showcase-snippet inconsistency it created (U9). Filed the two remaining structurally-untestable diagnostic gaps (Windows reserve-path counters, `page_size()`'s guard rejection branch) as a new tracked item 50, record-only (U10, U11).

**Round-8 closing review findings (UC1-UC6, `docs/reviews/2026-08-13-aligned-vmem-round8-closing-review.md`), all fixed in this same pass:**

- **UC1 (MEDIUM, fixed — this entry).** Round 8 had no CHANGELOG entry — the eighth recurrence of the gap item 1 exists to track, and item 1's own counter (just updated by round 8's own U8 fix to say "seven") was stale again the moment the round closed. This entry closes it; item 1's counter bumped to record the 8th instance.
- **UC2 (LOW-MEDIUM, fixed).** The round-8 review doc was left uncommitted while item 50 cited it three times, including in its `Evidence` field — TC2 recurring one round later. Investigation found this is not actually a contradiction needing resolution: the aligned-vmem campaign (this campaign) has committed its review docs since round 3, while a SEPARATE root-crate campaign (R34-2/task #521) explicitly does not commit its own — two correctly-scoped conventions for two different artifact classes, not one conflicting rule. Documented as a new item 2 in `docs/CORRECTNESS_OPEN_ITEMS.md` so a future round does not re-investigate from scratch; both round-8 review docs committed in this same commit.
- **UC3 (LOW, fixed).** Item 50's U11 half was stale on arrival: it asked "if round 8's U1 fix … lands" — five merges after it had already landed, in the same round. Reworded to past tense, both affected bullets.
- **UC4 (LOW, fixed).** Task #902's new `decommit_contract_violation_never_reaches_madvise` test named a counterfactual (a future `page_size()` → `PAGE` validation-base swap) in its doc comment that the offsets it actually passed could not detect under either base — round 7's T2 shape (a test whose doc comment claims coverage the assertion doesn't deliver), recurring in the very test written to close a coverage finding. Added a third, real discriminating call gated on `page_size() > PAGE` (a no-op on 4 KiB hosts, live on the macOS CI runner) so the test now genuinely catches the swap it claims to guard against.
- **UC5 (LOW, fixed as a one-off; filed as item 51 for future rounds).** The round's entire verification matrix ran on `x86_64-pc-windows-msvc`, which never compiles this crate's `#[cfg(unix)]` code — including U1's own fix and task #902's new Unix-only test. Ran `cargo check`/`cargo clippy --target x86_64-unknown-linux-gnu --all-targets` against the merged tree (both clean) as this round's missing verification step; filed as item 51 so future rounds touching `crates/vmem` add this to their own matrix rather than relying on a post-push CI discovery.
- **UC6 (record-only, no code change).** U1's fix ships with no regression-test backing; the closing review concluded this is the right call (a test for the fixed behaviour needs an injection seam CLAUDE.md's benchmark-hook rule discourages building; a test for the unchanged behaviour would never run in this repo's CI) and identified the Linux `bench-internals`-against-real-backend CI row (already tracked, item 48's S4 remainder) as now having three independent beneficiaries rather than one.

Round 8's only runtime change is U1's, independently re-verified correct by the closing review through call-graph tracing rather than taken on trust. Publish-readiness (task #658) is not newly blocked: `cargo package -p aligned-vmem --list` returns the same 20-file set as rounds 6/7, and U6's publish-facing attribution now matches its source card exactly.

#### `aligned-vmem` — round-9 follow-up (2026-08-13, tasks #906-907)

A fresh review of the post-`b6bfdac` tree (`docs/reviews/2026-08-13-aligned-vmem-round9-review.md`, findings V2-1, V2-2, V2-3) found **zero pre-existing code defects** — the campaign's first effectively-null full-crate read. All three findings lived entirely in text round 8's own closing commit (`b39882b`) had written one commit earlier: V2-1 (LOW) — round 8's UC4 fix added a discriminating call for `decommit`'s validation-base guard but not for `decommit_lazy`'s, while the test's own doc comment and assert message claimed to cover the guard PAIR; V2-2 (LOW) — a code comment still named `try_reserve_aligned_exact` as a `page_size()` caller after round 8's U1 fix had removed that call entirely; V2-3 (INFO) — a trailing comment said "both calls" after V2-1's own fix (task #906) would make it three. The review's own explicit verdict was **PIVOT**: given three consecutive rounds (7, 8, 9) of full-crate reads finding progressively less in the crate itself and progressively more in the previous round's own remediation text, it recommended ending the scheduled full-read cycle in favor of event-driven diff-scoped review plus targeted infrastructure investment (a Linux CI row running `bench-internals` against the real non-mock backend, item 41's miri step, item 43's BSD verification, and a citation-resolver script). This round needed only 2 small tasks (not the usual 7), each delegated to an independent `Agent(subagent_type: "sh")` in an isolated worktree, personally zero-trust reviewed and merged.

- **[test(vmem), P2] Task #906 (merge `486a97b`) — V2-1+V2-3: added `decommit_lazy`'s missing discriminating call, fixed a stale-count comment.** Added `decommit_lazy(base, PAGE, 2 * PAGE)` inside the existing `page_size() > PAGE`-gated block in `smoke.rs`'s `decommit_contract_violation_never_reaches_madvise`, alongside `decommit`'s existing call, so the test now genuinely discriminates BOTH guards' validation base rather than just one. Bonus: mirrored the same discrimination at the mock call-log layer in `mock.rs`'s `decommit_silently_skips_contract_violating_offsets`, giving both guards a second oracle on the `--all-features` CI row. Reworded "both calls" to "all calls above" so the trailing comment stays correct at any future call count.
- **[docs(vmem), P3] Task #907 (merge `3900828`) — V2-2: fixed a stale caller-inventory comment.** A NOTE under `query_os_page_size`'s Windows-arm `debug_assert!` still listed `try_reserve_aligned_exact` as a caller of `page_size()`; round 8's task #897 (U1) had already removed that function's only call to `page_size()` one round earlier. Removed the stale clause; verified mechanically (`grep -n "page_size()" crates/vmem/src/lib.rs`) that the only remaining runtime callers are `decommit`/`decommit_lazy`.

**Round-9 closing review findings (V2C1-V2C7, `docs/reviews/2026-08-13-aligned-vmem-round9-closing-review.md`), all fixed in this same pass:**

- **V2C1 (LOW, fixed).** A test doc-comment citation (`lib.rs:955-967`, "the literal at `:963`") that named where `granted_huge: false` lives had been accurate when written (round 7, task #892) but drifted stale within ONE round and had since survived TWO further full-crate reads (rounds 8 and 9) undetected — this round's own `+2` line shift (task #907) finally pushed the cited literal entirely outside the cited range, making the drift visible. Replaced with a symbol-based description (naming the closure, not a line range), immune to future drift; gave the neighboring `huge_pages.rs:61-62` citation the same treatment.
- **V2C2+V2C5 (LOW/INFO, fixed together).** Task #906's own bonus mock-layer mirror only covered `decommit_lazy`'s guard, not `decommit`'s, while the test's doc comment named `decommit` as the thing being locked (V2C5); and a neighboring test's "the mock suite has no way to observe this" claim became false the moment task #906's mirror landed, in the same commit (V2C2). Fixed by adding the missing symmetric `decommit` arm to the mock-layer mirror (making both guards' base swap observable at both layers), then narrowing the "no way to observe" claim to its now-true scope (cannot observe the real `madvise(2)` EINVAL specifically, since the mock layer never calls it — but the base-swap forwarding itself is now observable via the mock arm).
- **V2C3 (INFO, fixed).** Item 50's `lib.rs:409-445` citation for `query_os_page_size`'s three `#[cfg]` arms was exact when the round-9 review re-verified it, then task #907's `+2` insertion (in the SAME round) truncated it by 2 lines — the review's own "no `src/` file was touched" reasoning for clearing this citation was correct for round 8, and became wrong the moment round 9's own remediation landed. Replaced with a symbol-name citation.
- **V2C4 (INFO, fixed).** Task #906's own bonus comment in `mock.rs` said "the two `decommit_lazy` shapes above" when only one of the two calls above it was actually a `decommit_lazy` call (the other was `decommit`). Fixed in the same edit as V2C2/V2C5: reworded to correctly attribute one call to each function.
- **V2C6/V2C7 (process, fixed — this entry).** No CHANGELOG entry for round 9 (the ninth recurrence of item 1's tracked gap — a fourth consecutive round, 6-9, caught only by its closing review); item 1's counter bumped accordingly. Both round-9 review docs committed in this same commit, per the item-2 convention this campaign settled at round 8.

**On the PIVOT verdict:** the closing review's own independent assessment agreed with ending the scheduled full-read cycle, but conditionally — it found that its own two most durable findings (V2C1, V2C3) were citations located OUTSIDE this round's diff that this round's diff invalidated, the single most frequent finding class across the whole campaign (F11, T5, TC4, U2, U3, U8, V2-2, V2C1, V2C3) — and noted that a diff-scoped review (the pivot's proposed replacement) structurally cannot catch this class, since the defect and the commit that causes it are never in the same file region. Its recommendation: end the scheduled full reads, but build the citation-resolver script FIRST rather than leaving it as optional infrastructure — it is the only proposed mechanism that catches this class at all after the pivot. Neither of this round's two new test arms has executed anywhere yet (both are skipped on every 4 KiB host, live only on the macOS CI runner) — confirming they actually work as designed is owed before the campaign closes, the same way round 8's UC4 arm was only confirmed via the real CI job log after pushing.

#### `aligned-vmem` — round 10: independent review, deliberately outside the campaign's own methodology (2026-08-13, tasks #909-912)

Round 9's own closing review recommended pivoting away from full-crate re-reads on the grounds that 3 consecutive rounds of the SAME methodology (an LLM agent reading the whole crate) had converged on diminishing returns and were mostly finding defects in the campaign's own prior remediation, not in the crate. Per that recommendation, this round used a genuinely independent methodology instead: a fresh, self-contained review prompt (no summary of prior rounds' specific conclusions, explicit permission to disagree with 9 rounds of established consensus) was handed to a separate reviewer with no memory of this campaign's history, producing `docs/reviews/2026-08-13-aligned-vmem-independent-review.md`.

**The result falsified the pivot's own premise for reading the code again: the fresh methodology found one HIGH-severity defect that 9 rounds of the established methodology never found**, precisely because it had converged on the *same* blind spot across all 9 rounds — the huge-page leak fix (task #714, round 1) was re-examined and re-confirmed by multiple later reviews (round-1 audit, round-4 closing review), but every single one of those checks verified the ARITHMETIC built on top of a constant (`LINUX_HUGE_PAGE_SIZE = 2 MiB`) without ever questioning whether that constant matches what the OS actually does. A reviewer with a different framing checked the premise itself and found it false: Linux's plain `MAP_HUGETLB` (no size-encoding flag) requests the system's *configurable* default huge-page size, not universally 2 MiB — a `default_hugepagesz=1G` host (a normal HPC/database configuration) would leak the entire 1 GiB mapping and its pinned physical huge page on every release, because this crate's own `munmap` call used the wrong (2 MiB, not 1 GiB) length.

Two more real, verified defects were found in the same pass (M1: `Reservation::from_raw_parts` silently accepted two documented-forbidden `reservation_len` values its own doc comment falsely claimed were rejected; M2: the hand-written `mmap` FFI declaration had a real ABI shape mismatch risk on 32-bit Unix targets, not just a value-range non-issue as previously argued), plus one benchmark-integrity issue (L1: the bench harness recorded OOM/no-op iterations as valid fast samples). All four were personally zero-trust verified against the actual code before being decomposed into tasks — each finding's claimed mechanism was traced through the real call chain, not taken on the review's word.

Delegated to 4 parallel `/crush` sessions in isolated git worktrees (the first use of `/crush` rather than `Agent(subagent_type: "sh")` in this campaign's worktree-delegation pattern), each personally zero-trust reviewed and merged. Two tasks (H1, M2) both added a new item to `docs/CORRECTNESS_OPEN_ITEMS.md` and picked the same number (52) independently in their isolated worktrees — a genuine merge conflict, resolved by renumbering H1 to item 53 and keeping both items' full content, not a silent drop.

- **[fix, P0] Task #909 (merge `c3850e7`) — H1 (HIGH): fixed the Linux HugeTLB mapping leak.** Added a `MAP_HUGE_2MB` constant (`21 << MAP_HUGE_SHIFT` where `MAP_HUGE_SHIFT = 26`, evaluating to `0x54000000` — computed and verified independently against the Linux kernel's `include/uapi/linux/mman.h`, not copied from the task prompt's own initial arithmetic, which was wrong and got corrected in the process), OR'd into the `mmap` flags alongside `MAP_HUGETLB` so the crate's request now matches the 2 MiB constant it already validates against, regardless of the host's configured default. Fails closed: if 2 MiB huge pages aren't available, the mmap call fails cleanly rather than silently mapping the wrong size. REASONED-FROM-SPEC, not empirically verified on a real non-2-MiB-default host (none is in this project's CI) — recorded honestly as such. Filed and closed as item 53.
- **[fix] Task #910 (merge `da643f2`) — M1 (MEDIUM): fixed `from_raw_parts` accepting forbidden `reservation_len` values.** Added the two explicit predicates (`!= 0`, `is_multiple_of(PAGE)`) the constructor's own doc comment falsely claimed were already covered by the `Layout::from_size_align` check. Added two negative regression tests; personally verified counterfactually (temporarily removed the new predicates, confirmed both tests fail for the right reason, restored them).
- **[fix] Task #911 (merge `806cb30`) — M2 (MEDIUM): restricted the `mmap` FFI declaration's `off_t=i64` assumption to confirmed 64-bit-off_t targets.** Added a `compile_error!` rejecting all `unix + 32-bit-pointer-width` targets rather than special-casing any as safe, making a previously-only-documented restriction enforced at compile time. Filed as item 52.
- **[test] Task #912 (merge `3beb34a`) — L1 (LOW): made the benchmark harness fail closed.** Reserve/recommit failures now panic with a named-operation message instead of being silently recorded as valid (and misleadingly fast) samples. Not CI-relevant (benches aren't executed in CI, only compiled) but prevents a human misreading a local `cargo bench` run.

L2 (LOW, already tracked as item 48's S4 remainder) was confirmed by the independent review, not newly found — no new task needed.

Verification after all four merges: 44/49 tests (up from 42/47 — the two new M1 negative tests), four clippy rows (default / named feature set / `--all-features` / the Linux cross-compile row per item 51) clean, `cargo bench --no-run` clean, `cargo fmt --check` clean, doc-drift guard clean, root crate (`sefer-alloc`) still builds. No conflict-marker residue after the item-numbering conflict was resolved.
