...
//! `crate::alloc_core::size_classes::*` call site already uses, so nothing else changed.
//!
...
//!   policy parameter of the scheme) — it is `crate::alloc_core::os::SEGMENT`.
...
//!   (`crate::alloc_core::os::SEGMENT`, 4 MiB -- see `os.rs`), AND `carve_block` places
...
/// power of two `>=` [`crate::alloc_core::node::NODE_SIZE`] (the free-list node word — the
...
/// scheme's [`Params::huge_threshold`]. `crate::alloc_core::os::SEGMENT` — anything needing
/// a whole segment or more is "huge".
pub(crate) const HUGE_THRESHOLD: usize = crate::alloc_core::os::SEGMENT;
...
impl SizeClasses {
    ...
    /// exists solely so [`SegmentLayout::try_class_for`](crate::alloc_core::SegmentLayout::try_class_for),
