...
/// - **Unix** (native): calls `getenv(3)` and copies into `buf`.
/// - **Miri / any test that intercepts std::env**: falls back to
...
#[cfg(feature = "alloc-decommit")]
fn cfg_if_env_read(name_nul: &[u8], buf: &mut [u8]) -> usize {
    #[cfg(target_os = "windows")]
    // SAFETY: `name_nul` is a NUL-terminated ASCII C string constant.
    // `buf` is a stack-allocated slice; GetEnvironmentVariableA writes at most
    // `nSize` chars including NUL, so it cannot overflow `buf`.
    return unsafe {
        // GetEnvironmentVariableA: kernel32, cdecl on all Win32.
    ...
    // SAFETY: `name_nul` is a NUL-terminated C string; `getenv` returns a
    // pointer into the process environment block (or NULL). The pointed-to
    // string is valid for the process lifetime unless the environment is
    // modified concurrently — but Rust's `std::env` takes a lock on Unix,
    // so concurrent modification is excluded during normal use. We only READ
    // the returned value, never write it.
    unsafe {
        extern "C" {
            fn getenv(name: *const u8) -> *const u8;
        }
        let val = getenv(name_nul.as_ptr());
        if val.is_null() {
            return 0;
        }
        // Find the length by scanning for NUL (no libc strlen needed).
        let mut vlen = 0usize;
        while *val.add(vlen) != 0 {
            vlen += 1;
            if vlen > buf.len() - 1 {
                break;
            }
        }
        let n = vlen.min(buf.len() - 1);
        core::ptr::copy_nonoverlapping(val, buf.as_mut_ptr(), n);
        buf[n] = 0;
        n
    }
}
