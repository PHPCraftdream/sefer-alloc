...
impl AllocCore {
    ...
    #[cfg(feature = "alloc-decommit")]
    #[inline]
    pub(in crate::alloc_core) fn large_cache_scan_bound(&self) -> CombinedSlot {
        #[cfg(feature = "large-cache-extended")]
        {
    ...
    #[allow(unsafe_code)] // R14-1 (task #286): calls the `unsafe fn deref_large_cache_extension`
                          ...
                          // outlive this call.
    pub(in crate::alloc_core) fn large_cache_slot_get(
        &self,
        idx: CombinedSlot,
    ) -> Option<&CachedLarge> {
    ...
    pub(in crate::alloc_core) fn large_cache_slot_take(
        &mut self,
        idx: CombinedSlot,
    ) -> CachedLarge {
        if idx < LARGE_CACHE_SLOTS {
    ...
    #[cfg(feature = "alloc-decommit")]
    #[inline]
    pub(in crate::alloc_core) fn large_cache_deposit_budget_infeasible(
        &self,
        usable_size: usize,
    ) -> bool {
    ...
    pub(in crate::alloc_core) fn large_cache_find_free_slot(&mut self) -> Option<CombinedSlot> {
    ...
    #[allow(unsafe_code)] // R14-1 (task #286): calls the `unsafe fn deref_large_cache_extension_mut`
                          ...
                          // concurrent reader/writer, and no other reference to the sidecar is
                          // live across this call.
    pub(in crate::alloc_core) fn large_cache_slot_set(
        &mut self,
        idx: CombinedSlot,
        entry: CachedLarge,
    ) {
    ...
    #[cfg(feature = "alloc-decommit")]
    pub(in crate::alloc_core) fn maybe_decay_large_cache(&mut self) {
