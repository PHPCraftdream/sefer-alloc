...
//! MPSC stack for Large/huge segments, extracted so the
...
//! ownership), and its VALUE is the head of this MPSC stack of Large
...
//!
//! This is a cross-allocation intrusive stack: segment `A`'s
...
//! `expose_provenance` on the pointer actually returned by `head.swap` before packing its address
...
//! A per-node publishing sentinel prevents the owner from reading the link
//! before the producer stores that actual predecessor with Release.
