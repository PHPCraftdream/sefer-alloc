...
| `once-ptr-cell` | [![Crates.io](https://img.shields.io/crates/v/once-ptr-cell.svg)](https://crates.io/crates/once-ptr-cell) | [![Documentation](https://docs.rs/once-ptr-cell/badge.svg)](https://docs.rs/once-ptr-cell) |
...
| `once-ptr-cell` | `crates/once-ptr-cell/` | `#![allow(unsafe_code)]` — single documented reason: `unsafe impl Send/Sync` for the `AtomicPtr`-backed cell + `NonNull::new_unchecked`; every site has `# Safety` / `// SAFETY:` |
...
| `tagged-index-stack` | `crates/tagged-index-stack/` | `#![deny(unsafe_code)]` — exactly EIGHT audited, item-scoped `#[allow(unsafe_code)]` lint-exception regions, all in its production library source (`src/imp.rs`; its `tests/` are separate crate targets carrying intentional `unsafe impl StackStorage` test fixtures): the `unsafe trait StackStorage` declaration (whose three hooks are `unsafe fn`), the `SealedStorage` trait/bridge surface (its `store_next` member is `unsafe fn`, forwarded verbatim), and the caller-facing push boundary (`push_index`/`push_index_impl`/`ArrayIndexStack::push`, all `unsafe fn` under a three-clause link-domain + liveness + exclusive-ownership contract) (all item-scoped tier-2 allows, counted in the tier-2 table below); implementor obligations live in the trait's `# Safety` doc and each hook/push entry point carries its own caller-side `# Safety` contract (allocator consumers rely on its exclusive-issuance contract — `GlobalAlloc` category). Lock-free via a single packed `AtomicU64` head word; ABA tag in the high bits, no raw-pointer derefs |
...
| [`src/alloc_core/dirty_by_class.rs`](src/alloc_core/dirty_by_class.rs) | The lazily-materialised per-(segment, class) dirty-bit sidecar (`PerClassDirty`); dereferences the `OncePtrCell`-published sidecar pointer | `class-aware-dirty` |
| [`src/alloc_core/large_cache_extended.rs`](src/alloc_core/large_cache_extended.rs) | The lazily-materialised large-cache extension sidecar (owner-only, no `OncePtrCell` — no cross-thread publisher); reserves via `alloc_core::sidecar::reserve`, dereferences via `sidecar::deref[_mut]` | `large-cache-extended` |
...
| [`crates/tagged-index-stack/src/imp.rs`](crates/tagged-index-stack/src/imp.rs) | 8 | The `unsafe trait StackStorage` declaration (whose three hooks are `unsafe fn`; 2026-09-01 conversion + 2026-09-02 unsafe-fn hooks) AND the crate-private `SealedStorage` bridge impl — the hooks' sole call site, three `unsafe` blocks with per-call `// SAFETY:` proofs — AND (2026-09-02, review run 7) the caller-facing push boundary (`StackOps::push_index`, `push_index_impl`, `ArrayIndexStack::push`, all `unsafe fn` under a three-clause link-domain + liveness + exclusive-ownership contract) AND `SealedStorage::store_next`'s trait declaration — under eight item-scoped allows (ADR `docs/adr/2026-09-01-tagged-index-stack-storage-binding-closure.md`); the library target is otherwise `#![deny(unsafe_code)]` (its integration tests are separate crate targets carrying intentional `unsafe impl` test fixtures); the contract an `unsafe impl`/`unsafe fn` caller must uphold is the trait's `# Safety` doc and each unsafe fn's own `# Safety` section |
| [`crates/tagged-index-stack/scripts/tis_p3_ab/harness_bin.rs`](crates/tagged-index-stack/scripts/tis_p3_ab/harness_bin.rs) | 3 | Sol-codex review run 8, P2-2: the wall-clock A/B measurement harness template's three `stack.push(...)` call sites, each a statement-scoped `unsafe {}` block with its own `// SAFETY:` proof (domain + liveness + exclusive ownership) — the template's own `#![deny(unsafe_code)]` stays crate-wide, only these three sites are locally allowed; not part of the published LIBRARY TARGET, though it DOES ship in the published `.crate`'s file list (verified via `cargo package --list -p tagged-index-stack`); this file is a scratch-crate generator template materialised by `scripts/tis_p3_ab_runner.mjs` |

That's the full list (both tiers): **18** tier-1 module-level seams (12 in
`src/`, 6 in `crates/`) plus **84** tier-2 item-scoped allows across **20**
