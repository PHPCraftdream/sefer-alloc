...
| [`src/registry/heap_core_dealloc_batch.rs`](src/registry/heap_core_dealloc_batch.rs) | 14 | `dealloc_batch` / `dealloc_batch_small` — `unsafe fn` boundaries (caller-pointer contract) + internal call-site blocks into scalar `dealloc` / `AllocCore::flush_class` (R11-4); **plus** the R26-7 (task #416) `dbg_dealloc_batch_lazy` / `dealloc_batch_small_lazy` bench-only A/B pair — a byte-for-byte copy gated `bench-internals` (NOT reachable from plain `--features production`) whose only difference is a lazily-materialized `Option<[*mut u8; STAGE_CAP]>` staging array (7 sites: 2 `unsafe fn` boundaries + ownership-gate / mid-loop-flush / final-flush / entry-dispatch / scalar-fallback call-site blocks) |
| [`src/registry/heap_core_diag.rs`](src/registry/heap_core_diag.rs) | 4 | `dbg_push_to_ring` / `dbg_push_coarse_only_entry` (R13-1, gated `bench-internals`) / `dbg_dealloc_own_thread_with_base` (R23-3, task #372, gated `bench-internals`) / `dbg_overflow_bitmap_clear_pass` (R24-2, task #380, re-gated `bench-internals` + made `unsafe fn` by R25-1, task #395) — `unsafe fn` boundaries (delegation to the unsafe producer / documented raw-pointer contract); see the R24-6/R25-1 note below the table |
...
`src/`, 7 in `crates/`) plus **69** tier-2 item-scoped allows across **17**
...
dbg_*` hooks in `heap_core_diag.rs`.** All four of that file's `unsafe fn`
...
from any production alloc path — none of the four changes what
...
- `dbg_dealloc_own_thread_with_base` (R23-3, task #372),
  `dbg_push_coarse_only_entry` (R13-1, task #271), and
  `dbg_overflow_bitmap_clear_pass` (R24-2, task #380) were all reachable from
  a plain `production` build (their prior gates — `alloc-global + fastbin`,
  `alloc-xthread + alloc-segment-directory + class-aware-dirty`, and
  `alloc-global + fastbin` respectively — are each a subset of `production`'s
  feature list). `dbg_overflow_bitmap_clear_pass` was additionally a **safe**
  `pub fn` despite deriving a segment base from an unvalidated caller pointer
  and writing allocator metadata through it — a genuine safe-code-reachable
  soundness hole (flagged by the R24 readonly review,
  `docs/reviews/2026-07-28-r24-readonly-review.md`), not just a feature-gating
  concern like its two siblings; R25-1 (task #395) fixed it by making it
  `pub unsafe fn` with a documented `# Safety` contract, in addition to the
  same `bench-internals` re-gating the other two received. Each of the three
  has exactly ONE caller in the whole tree (`benches/perf_gate_iai.rs` for
  two of them, `tests/class_aware_dirty_oom_latch.rs` for the third), so all
  three are now additionally gated behind the `bench-internals` feature (see
  the feature table above) — a plain `--features production` build no longer
  compiles any of them in.
- `dbg_push_to_ring` (`HeapCore`/`AllocCore`, R6-MS-4) is **deliberately left
  ...
  the three above it is called from ~20 files across the entire
  ...
  from any "changes production behavior" claim, exactly like its three
...
| `bench-internals` | — | R24-6 / R25-1: gates the smallest set of `unsafe fn dbg_*` hooks that exist ONLY to let `benches/perf_gate_iai.rs` / one integration test isolate a perf-gate sub-cost, and whose own `#[cfg]` would otherwise be fully satisfied by `production` alone (`HeapCore::dbg_dealloc_own_thread_with_base`, `HeapCore::dbg_push_coarse_only_entry`, `HeapCore::dbg_overflow_bitmap_clear_pass`). **NOT** in `production` — a plain `--features production` build of the library never compiles these three hooks in. Carries no code of its own. | off | never, in application code — internal to this crate's own CI perf-gate and one test |
