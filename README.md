...
`unsafe` lives in **eight named confined seams** (`alloc_core::platform::{os,
...
| `globalalloc-model` | `crates/globalalloc-model/` | `#![allow(unsafe_code)]` — two documented reasons: (a) the `unsafe trait RawAllocator` (its impls must return valid pointers for the requested layout), every impl + call carrying `// SAFETY:`; (b) `DoubleFreeOk::new`, the unforgeable consent token for the M2 double-free oracle, an `unsafe fn` declaration carrying its own `# Safety` contract |
...
The two A/B templates below are repository-only and excluded from the
published package under `scripts/`; the canonical grep still counts their
item-scoped regions.
...
measurement-only `unsafe fn dbg_*` hooks in `heap_core/diag/`.** All seven
of those files' `unsafe fn` entries above are `#[doc(hidden)]`, exist ONLY to
...
[`src/alloc_core/platform/numa.rs`](src/alloc_core/platform/numa.rs) (Linux `mbind` via
...
a test file, and a reproducible command. **278 integration test files** ship
...
|---|---|---|
| Unit / integration tests | Construction, edge cases, end-to-end behaviour | `tests/*.rs` (278 files) |
...
- **A hard cap on simultaneously-LIVE Large objects: `MAX_SEGMENTS - 1`
  ...
  (`src/alloc_core/segment/segment_table/mod.rs`). Slot 0 is permanently reserved for
