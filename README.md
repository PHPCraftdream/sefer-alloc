...
| [`crates/tagged-index-stack/scripts/tis_p3_ab/harness_bin.rs`](crates/tagged-index-stack/scripts/tis_p3_ab/harness_bin.rs) | 3 | Sol-codex review run 8, P2-2: the wall-clock A/B measurement harness template's three `stack.push(...)` call sites, each a statement-scoped `unsafe {}` block with its own `// SAFETY:` proof (domain + liveness) — the template's own `#![deny(unsafe_code)]` stays crate-wide, only these three sites are locally allowed; not part of the published crate, this file is a scratch-crate generator template materialised by `scripts/tis_p3_ab_runner.mjs` |
