...
//! `(pool_segments=4, pool_byte_cap=16 MiB)` — the CURRENT actual default (a
//! genuine same-vs-current-default baseline). Runs the EXACT
//! `bench_global_alloc_churn_with_teardown`@1024B shape R25-5/R26-1/R26-3
//! measured, driven through the REAL `#[global_allocator]` SeferAlloc, at the
//! REAL byte cap (NOT R26-3's 256 MiB ceiling). Since `pool_cap` resolves as
//! `min(pool_segments, pool_byte_cap/SEGMENT)`, R26-3's 256 MiB and this arm's
//! 16 MiB both resolve to effective cap 4 — so this proves the real byte cap
//! does not unexpectedly bind.
//!
...
//!
//! ```text
//! cargo build --release --example r27_4_real_default_ab_cap4 --features "production alloc-stats"
//! ```
...
/// `pool_segments = 4` — current DEFAULT_POOL_SEGMENTS baseline, paired with the
/// REAL current DEFAULT_POOL_BYTE_CAP of 16 MiB (resolves to effective cap 4).
const POOL_SEGMENTS: usize = 4;

/// 16 MiB — the REAL current DEFAULT_POOL_BYTE_CAP (NOT R26-3's 256 MiB ceiling).
/// Resolves to effective cap min(4, 16 MiB / 4 MiB) = 4.
const REAL_POOL_BYTE_CAP: usize = 16 * 1024 * 1024;
