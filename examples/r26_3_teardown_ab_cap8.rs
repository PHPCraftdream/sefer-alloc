//! R26-3 (task #412) — A/B arm: `pool_segments = 8` (the GO-CANDIDATE raise
//! R25-5/R26-1 identified; the current `DEFAULT_POOL_SEGMENTS` is still 4).
//! Runs the EXACT `bench_global_alloc_churn_with_teardown`@1024B shape
//! R25-5/R26-1 measured, driven through the REAL installed
//! `#[global_allocator]` SeferAlloc (via `std::alloc::alloc`/`dealloc`), NOT
//! `AllocCore` directly. R25-5/R26-1 measured the latency/decommit axis via an
//! `AllocCore::new_with_config` bypass; THIS binary confirms the same finding
//! (cap 4->8 eliminates the decommit-driven slowdown) through the un-bypassed
//! production entry point, paired against its cap=4 sibling by
//! `scripts/paired-ab-runner.mjs`.
//!
...
//! difference vs `r26_3_teardown_ab_cap4.rs` is `POOL_SEGMENTS`.
//!
//! ## Run
//!
//! ```text
//! cargo build --release --example r26_3_teardown_ab_cap8 --features "production alloc-stats"
//! ```
...
/// `pool_segments = 8` — the GO-CANDIDATE raise R25-5/R26-1 identified. At
/// cap=8 the ~6-segment working-set demand is absorbed with zero decommits.
/// (The current `DEFAULT_POOL_SEGMENTS` is still 4; this task does NOT change it.)
